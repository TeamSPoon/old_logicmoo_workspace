...
% Invoke Unhandled
      ifThen_API(not(Out=[_|_]),(muh(clientEventUnhandled(Channel,Agent,CMD)),findall(1,safetyEach(clientEventUnhandled(Channel,Agent,CMD)),_))),!.

muh(X):-writeSTDERR(X),sayLine("#kif",[X]).

ifThen_API(Test,Then):-ignore((Test,!,Then)).

safetyEach(X):-catch(X,E,(fmt(user_error,'% ~q from ~q.\n',[E,X]),fail)).

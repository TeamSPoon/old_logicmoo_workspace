...
e2c(Event,English,CycL9):-
     ...
     concat_atom(English,' ',Text),fmt('~n?- e2c("~w").~n~n', [Text]),flush_output,
      ...
      %%ignore(catch((englishCtx(Event,CycL,Tagged,[])),E1,writeCycL(E1))),!.
...
utterance(Event,CycL) -->['S'|Form],{!,debugOnFailure(phrase(utterance(Event,CycL),Form))}.
%utterance(Event,template(TempType,CycL)) --> applySTemplate(TempType,CycL).
...
%noun_unit(Type,Event,Subj,CycL0,CycL) --> noun_unit(Type,Event,Subj,CycL5,CycL),np_post(Event,Subj,CycL0,CycL5).
...
aux(AVP) --> thePOS(AVP,'Modal').
aux(AVP) --> thePOS(AVP,'AuxVerb').
...
thePrep(Event2,'PrepAFn'(P,V))-->aux(V),{!},thePrep(Event2,P).
...
chunkParseCycL(English,Event,Subj,Tagged,['TheList'|CycL9]):-
...
%% maybe noun_unit_start(Type,Event,Subj,Scope,CycL).
noun_unit(Type,Event,Subj,CycLIn,and(CycL,CycLIn)) -->noun_unit_start(Type,Event,Subj,Scope,CycL). 
...
% $100
%was noun_unit(Type,Event,Subj,CycLIn,and(equals(Subj,'DollarFn'(Num)),CycLIn))

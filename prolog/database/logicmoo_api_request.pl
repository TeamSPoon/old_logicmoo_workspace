...
% =================================================
clientEventUnhandled(Channel,Agent,X):- once(channel_fmt('"#logicmoo"',clientEventUnhandled(Channel,Agent,X))),fail.
clientEventUnhandled(Channel,Agent,X):-
   memberchk(file=_,X),
   isMooOption(Agent,client,html),!,
   writeHTMLStdHeader('404 handler'),
   fmt('~q<br><p>Welcome to LogicMoo is the request page handler for non-understood requests',[X]),
   showMooProcessHTML,
   writeHTMLStdFooter.

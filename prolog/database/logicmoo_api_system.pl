...
/* 
% mud defined 
logOnFailure(X):-catch(X,E,(fmt(error(X,E)),!,fail)),!.
logOnFailure(X):-fmt(falure(X)),!,fail.
*/
...
/* 
% mud defined 
fmt(Channel,Format,Args):-catch((fmt(Channel,Format,Args),flush_output(Channel)),_,true).
fmt(Format,Args):-catch((fmt(Format,Args),flush_output),_,true).
fmt(Format):-catch((fmt(Format,[]),flush_output),_,true).
*/
...
writeErrMsg(Out,E):-!. %message_to_string(E,S),fmt(Out,'<prolog:error>~s</prolog:error>\n',[S]),!.
writeErrMsg(Out,E,Goal):-!. %message_to_string(E,S),fmt(Out,'<prolog:error>goal "~q" ~s</prolog:error>\n',[Goal,S]),!.
writeFileToStream(Dest,Filename):-
        ...
        close(Input)),E,
        fmt('<prolog:error goal="~q">~w</prolog:error>\n',[writeFileToStream(Dest,Filename),E])).
...
moduleEnsureLoaded(X):-
        ...
        ensure_loaded(Y)),_,fmt(user_error,';; file find error ~q ~q',[X,E])))).
...
showMooStatisticsHTML:-
   fmt('<pre>'),prolog_statistics,fmt('</pre>').
...
writeMooProcessesHTML(nokill,Name,Goal,Id,Options,Status):-
        fmt('<tr><td>~w</td><td><nobr>~w</td><td>~w</td><td>&nbsp;</a></td><td>~w</td><td>~w</td><tr>\n ',[Id,Name,Status,Options,Goal]),!.

writeMooProcessesHTML(Perms,Name,Goal,Id,Options,Status):-
        fmt('<tr><td>~w</td><td><nobr>~w</td><td>~w</td><td><A href="system.moo?killable=~w">Kill</a></td><td>~w</td><td>~w</td><tr>\n ',[Id,Name,Status,Id,Options,Goal]),!.
...
bot(ps):-!,mooThreadCreate_data(Perms,Name,Goal,Id,Options),
	prolog_current_thread(Id,Status),
	fmt((Id:Name:Goal:(Status):(Perms):Options)),fail.
...
yn(P):-catch(P,E,fmt(E)),!,fmt(yes).
yn(P):-fmt(no).

timeit(X):-getCputime(S),yn(X),getCputime(E),Time is E-S,fmt(Time).
mtry(G):-catch(G,E,fmt(E)).
...
pp(H):-findall(Y,predicate_property(H,Y),L),L=[_|_],fmt(L),!.

pp(H):-fmt(nv(['i don''t know about',H])),!.

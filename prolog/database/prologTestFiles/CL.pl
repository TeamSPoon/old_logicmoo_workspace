...
valid0(Grd,Parnum,Stack,Env,Ni,No):- 
    ...
    %missing ELSE part to main implication: model found with goal false!
                      
valid1(G,Parnum,Stack,E,Env,Ni,No) :- 
             numbervars(E,par,Parnum,Parnew), % instantiate
...
coq_prf(O,ap(Ax,Cond)) :- 
   Ax=or_elim(N) -> log(N,E,_,_,_),coq_ors(O,Cond),lem_prf(O,N)
;  Ax=ex_elim(N) -> log(N,E,_,_,_),nl,wl(O),coq_rel(O,Cond),lem_prf(O,N)

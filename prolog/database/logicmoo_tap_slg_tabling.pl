...
xhandle(E,Call):-fmt('xhandle: ~n~q.~nxhandle:',[E:Call]),true,fail.
...
mslg(Goal):-
	...
	length(L,N),
	T is  E - S,
	fmt('found ~w answers in ~q secs.~n',[N,T]),!.
...
prologCall(true(_),Call,F,Args,Body):-
	once((fmt('needs: '),writeArgLit(Call))),
	copy_term(Call,Copy),
	mooCache(Copy, Body,/*true(_)*/ _, Context, Ctx,surf(Context,TN,CLID,Vars)),
		not(recorded(TN,Call)),
		(recorda(TN,Call)),
		Call=Copy,
	once((fmt('found: ~q ~n',[[TN,CLID,Vars]]),writeArgLit(Call),writeArgLitL(Body))).
...
prologCall(false(_),Call,F,Args,Body):-
		...
		fmt('~q.~n',[true:X:surf(Context,TN,CLID,Vars)]).
				  */
...
converts clauses of tabled predicates into a certain fmt. For the
...
predicate in the body are kept in their original Prolog fmt. If

...
chose_or_create_character(Character):-
	...
	fmt(user_error,'% No Characters not found creating ~w.~n',[Character]),
...
chose_or_create_character(Character):-
	...
	fmt(user_error,'% Characters found: ~q~n',[List]),
	(best_member(Character,List,Name) ->
	    (wf(takeCharacter(Name)),!,fmt(user_error,'% Assuming: ~q~n',[Name]));
	    (mooInfo(player_login, PlayerLogin),fmt(user_error,'% Creating: ~q~n',[Character]),wf(createChar(PlayerLogin,Character)))),!.
...
flush_read_to_error:-
   readPackedTokChars(Data),!,
   ((Data= []) ; fmt(user_error,'~s~n~n',[Data])),!.
...
reactPackedTokChars([91|Data]):-!,
     ...
     fmt(user_error,'Error ~s~n~n',[[91|Data]])),!.

reactPackedTokChars(Data):-
      fmt(user_error,'~s.~n~n',[Data]).
...
reactPackedTok(asserta(mooInfo(Type,H))):-
   fmt(user_error,'~q~n',[asserta(mooInfo(Type,H))]),
...
reactPackedTok(X):-
   fmt(user_error,'% reactPackedTok(~q).~n',[X]).

...
clientEvent(Channel,Agent,[file='cycl.moo'|A]):-!,
	 ...
	 fmt('<form method="GET">'),
	 showContextHtml(Ctx),
	 fmt('<p><textarea rows="9" name="formula" cols="40">~w</textarea><br>
	 ...
	 fmt('~n<br><b>Total time was ~w secs</b>',[Total]),!,
	 writeHTMLStdFooter,!.
...
processRequestCycL('Query',Formula,Chars,Goal,Assert,Vars,Mt,IT):-
      ignore(catch((andCall(Goal),writeCycLVarBindings(Vars),fail),E,fmt('<br>~n~q',[E]))),!.
...
assertCycLWithTMS(Assert,Vars,Mt,IT):- 
	    ...
	    fmt('~n<font color=red>Assertion veto by Truth Compiler~nReason: Contains variables that are not quantified~nTry using quantifiers like #$forAll or #$thereExists</font>~n'),
...
fixDb:-see(logicmoo('mworld0.pldata')),
   repeat,
      read(Term),
...
tellall(Sig):-flag(Sig,_,0),Sig,fmt('~q.~n',[Sig]),flag(Sig,X,X+1),fail.
tellall(Sig):-!,flag(Sig,X,X),fmt(user_error,'% ~q total ~d.~n',[Sig,X]).
...
loadRCycTerm(X):-X=..[F|ARGS],
         ...
         NX=..[F|Left],
         catch(loadRCycTerm2(NX),E,(fmt(user_error,'~q.~n',[E]))),!.
...
writeAssertions(assertedMt(PredO,Arg1O,Arg2O,ArgSO,MT)):-!,
      fmt('~q.~n',[asserted(PredO,Arg1O,Arg2O,ArgSO)]).
...
asserted(A,B,C,D):-inferAsserted(A,B,C,D).
             ~n',[]),!,
      ignore((assertedLocal(PredO,Arg1O,Arg2O,ArgSO),fmt('~q.~n',[asserted(PredO,Arg1O,Arg2O,ArgSO)]),fail)),
      told,
      !. %['asserted.pl'].
...
cycQFull(X):- X=..[H|XS], transitive(genlPreds,L,H),dmsg(genlPreds(L,H)),cycQPred(L,XS).
cycQFull(C):-binPred(implies,A,C),not(contains_term(C,A)),dmsg(implies(A,C)),cycQFull(A).
...
my_exec(I,J):- catch('$toplevel':'$execute'(I, J),E,(fmt(user_error,'% ~q.~n',[E]),fail)).
...
qsizeV(F,X):-findall(F,X,L),length(L,N),sort(L,LS),length(LS,NS),fmt('% Size ~q = ~q:~q~n',[X,N,NS]),member(F,LS).
...
cycAssertBuffer(Out):-dmsg('% cycAssertion ~w ~n',[Out]),assertIfNew(cycCacheToDo(Out)),!.
...
cycSync(Out):-Out= MT : Assert,!,catch((myCycAssert(MT : Assert),ignore(assertIfNew(cycCache(Out))),ignore(retract(cycCacheToDo(Out)))),E,dmsg('%%%%%%%%%%%%% ~q',[E])),!.
...
getCycConnection(Server,SocketId,OutStream,InStream):-
      ...
      dmsg('Connected to Cyc TCP Server {~w,~w}\n',[InStream,OutStream]),
      assertz(cyc:cycConnectionUsed(Server,SocketId,OutStream,InStream)),!.
...
cycSyncThread:-
         ...
         dmsg(createProcessedGoal(cycSyncThreadCode)).
...
number_of_clauses(P):-number_of_clauses(P,Y),functor(P,F,A),dmsg('% number_of_clauses(~q/~q)->~q~n',[F,A,Y]).
...
writel(OutStream,N,Vars):-N==[],!,fmt(OutStream,'NIL~n',[]).
...
toCycApiExpression(fmt(S,List),Vars,Out):-!,toCycApiExpressionEach(List,OList),sformat(Out,S,OList),!.
...
getWordTokens(S,Y):-toCodeList(S,XS),once( tokenize3(XS,Y) ). %,dmsg('~q.~n',[Y]).
...
fmt(X,Y,Z):-catch((fmt(X,Y,Z),flush_output_safe(X)),_,true).
fmt(X,Y):-catch((fmt(X,Y),flush_output),_,true).
fmt(X):- once((atom(X) -> catch((fmt(X,[]),flush_output),_,true) ; fmt('~q~n',[X]))).

fmt(X,Y,Z):-catch(fmt(X,Y,Z),_,true).
fmt(X,Y):-fmt(X,Y).
fmt(X):-fmt(X,[]).
...
/*moduleEnsureLoaded(X):-
        ...
        ensure_loaded(Y)),_,dmsg(';; file find error ~q ~q',[X,E])))).
% ========================================================================================
% Platform specifics
% ========================================================================================
...
attemptServerBind(ServerSocket, Port):-
        ...
        dmsg('% CYC Prolog API server started on port ~w. \n',[Port])),
        error(E,_),
        dmsg('% CYC Prolog API server not started on port ~w becasue: "~w"\n',[Port,E])).
...
serviceAcceptedClientSocketAtThread(OClientInfo,ClientSocket,In,Out):-
   %    tcp_open_socket(ClientSocket,In,Out),!,      
       ClientInfo = ['io'(ClientSocket,In,Out)|OClientInfo],
        ...
        ignore(catch(serviceIO(ClientInfo,In,Out),E,dmsg(E:serviceIO(ClientInfo,In,Out)))),
...
%my_peek_char(In,Char):-dmsg(my_peek_char(In,Char)),peek_char(In,Char).

serviceIO(ClientInfo,In,Out):-
        peek_char(In,Char),!,
	dmsg('~q',serviceIOBasedOnChar([firstChar(Char)|ClientInfo],Char,In,Out)),
...
serviceIOBasedOnChar(ClientInfo,SkipChar,In,Out):- char_type(SkipChar,space),!,
	dmsg(serviceIOBasedOnChar(ClientInfo,SkipChar)),
        get_char(In,_),
        peek_char(In,Char),!,
	%dmsg(serviceIOBasedOnChar(ClientInfo,Char,In,Out)),
...
serviceCycApiRequest(ClientInfo,In,Out):-
   ...
   tell(Out),
 %  repeat,
	 once((readLispStream(In,PrologGoal,ToplevelVars),dmsg('remote API Call "~q" ~n',[PrologGoal]))),
	 ignore(once(catch(once(callCycApi(Out,PrologGoal,ToplevelVars)),E,(fmt(Out,'500 "~q"\n',[E]),fmt(user_error,'% sent cyc: 500 "~q"\n',[E]))))),
         !.
...
cycGoal(X,Y,Z):-dmsg('?- ~q.~n',[cycGoal(X,Y,Z)]),fail.
cycGoal([],Vars,[]):-!.
...
%cycGoal(X,Vars,Lisp):- dmsg(passIn(X)),passAlong(X,Y),toCycApiExpression(Y,Vars,Lisp),dmsg('\n% Pass back->'),dmsg(Lisp),!.
...
cycGoal(['PEVAL', string(EVAL)], _G94, Result):-
         ...
         dmsg('-> ~q.~n',[ToplevelVars:Result])),E,Result=[E]),!.
...
processRequest(X):-   
   catch(once(user:processRequestHook(X)),E,(dmsg('ERROR: processRequestHook: "~q" \n',[X:E]), fail)),!.
processRequest(X):-
        dmsg('FAILED: processRequestHook: "~q" \n',[X]),
...
serviceNativeRequestAsRDF(_,In,Out):-
        fmt(Out,'<?xml version="1.0" encoding="ISO-8859-1"?>\n',[]),
        thread_self(Session),
        retractall(isKeepAlive(Session)),
        xmlClearTags,
        repeat,
...
keep_alive:-thread_self(Me),retractall(isKeepAlive(Me)),assert(isKeepAlive(Me)),fmt('<keepalive/>\n',[]).
goodbye:-thread_self(Me),retractall(isKeepAlive(Me)),fmt('<bye/>/n',[]).
...
%               fmt(Out,'<cycml:no/>\n',[])),!,
        ...
        fmt(Out,'</cycml:solutions answers="~w" cputime="~g">\n',[Returns,Elapsed]),!.
...
throwCyc(Module,Type,Details):-
        ...
        ifInteractive(dmsg('Post throwCyc')),!.
...
serviceSoapRequest(In,Out):-
      dmsg('SOAP Request'),
        ...
        fmt(Out,'<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n<error>~w</error>\n',[E])),
        flush_output_safe(Out).
...
fmtServer(A,B):-fmt(A,B),flush.
fmtServer(O,A,B):-fmt(A,B),flush_output_safe(O).

xmlOpenTag(Name):-thread_self(Self),asserta(xmlCurrentOpenTags(Self,A)),fmtServer('<~w>',[Name]),!.
xmlOpenTagW(Out,Name,Text):-thread_self(Self),asserta(xmlCurrentOpenTags(Self,A)),fmtServer(Out,'~w',[Text]),!.

xmlCloseTag(Name):-thread_self(Self),ignore(retract(xmlCurrentOpenTags(Self,A))),fmtServer('</~w>',[Name]),!.
xmlCloseTagW(Name,Text):-thread_self(Self),ignore(retract(xmlCurrentOpenTags(Self,A))),fmtServer('~w',[Text]),!.
xmlCloseTagW(Out,Name,Text):-thread_self(Self),ignore(retract(xmlCurrentOpenTags(Self,A))),fmtServer(Out,'~w',[Text]),!.

xmlClearTags:-thread_self(Self),retractall(xmlCurrentOpenTags(Self,A)).

xmlExitTags:-thread_self(Self),retract(xmlCurrentOpenTags(Self,A)),fmtServer('</~w>',[Name]),fail.
xmlExitTags.
...
xml_assert(Show,Ctx,Context,User):-!,
        fmt('<assertionResponse accepted="false">\nUnable to parse: "~s"\n</assertionResponse>\n',[Show]).
        ...
        getMooOption(Agent,interp='cycl',Interp),
         ...
         logOnFailure(toCycApiExpression(STERM,NEWFORM)),!,
              logOnFailure(once(( NEWFORM=comment(_) ->
                     (fmt('<cycml:error>Syntax Error: Unmatched parentheses in "~s"</cycml:error>\n',[Show]),!,FORM=_) ;(!,
...
invoke_final_answer(possible,How,CPU):-!,
        fmt('<requestResponse yesno="~w" numBindings="0" seconds="~w"/>\n',[How,CPU]).

invoke_final_answer(Logic,How,CPU):-
        fmt('<requestResponse yesno="~w" numBindings="~w" seconds="~w">\n<bindings>\n',[Logic,How,CPU]),
...
cycPredCall(X,Y,[]):-attach_console,true,dmsg(cycPredCall(X,Y)),!.
...
inform_xml_agent(UResultsSoFar,Result,InExplaination,Status):-
        dmsg(inform_xml_agent(UResultsSoFar,Result,InExplaination,Status)),fail.
...
inform_nv(Name=Value,Vars):-
        ...
        fmt('<var varName="~w" value="~w"/>\n',[OName,OValue]).


inform_xml_explaination(InLength,ChoiceExplaination,Result):-
        fmt('<explaination numSteps="~w">',[InLength]),
...
%
% Message is a prolog term in the fmt defined by it's Class
% Each Class has a one known Message fmt shown in the table.   
...
%dmsg([-1]).
%dmsg([[-1]]).
%dmsg(T):- isMooOption(Agent,opt_debug=off),!.
dmsg(Stuff):-!,dmsg('% ~w~n',[Stuff]).
dmsg(T):-!,
	((
	if_prolog(swi,
...
%dmsg(C,T):- isMooOption(Agent,opt_debug=off),!.
dmsg(_,F):-F==[-1];F==[[-1]].
dmsg(F,A):-
        nl(user_error),
        fmt(user_error,F,A),
...
sendNote_1(canonicalizer,From,Subj,Message,Vars):-
            ...
            fmt('<font color=red>canonicalizer</font>: ~w "~w" (from ~w). \n',[SA,SS,SFrom]),!.
...
sendNote_1(To,From,Subj,Message,Vars):- isMooOption(Agent,client=automata),  !, 
            ...
            toMarkUp(cycl,nv(Subj),Vars,S),
            toMarkUp(cycl,nv(Message),Vars,A),
            fmt(user_error,'% ~w (~w from ~w) ',[A,S,SFrom]).
...
sendNote_1(To,From,Subj,Message,Vars):- isMooOption(Agent,client=html),  !, %  In Html
            ...
            fmt('<hr><B>To=<font color=green>~w</font> From=<font color=green>~w</font> Subj=<font color=green>~w</font></B><BR>~w\n',[To,From,S,A]),!.
...
writeCycEvent(request_end,(Result,Normal,Elapsed,Num,Bindings),Vars):-!, 
                  (isMooOption(Agent,client=html) -> 
                     ((    
                       ...
                       fmt('<Summary result="~w" solutions="~d" bindings="~d" cpu="~f"/>\n</Answer>\n',[Result,Num,Bindings,Elapsed]),
                       le_pull('Answer')
                     ));
...
showCycProcessHTML:-
        ...
        fmt('<hr><table border=1 width=80%><th>Id</th><th>Name</th><th>Status</th><th>Actions</th><th>Options</th><th>Goals</th>',[]),
...
writeCycProcessesHTML(nokill,Name,Goal,Id,Options,Status):-
        fmt('<tr><td>~w</td><td><nobr>~w</td><td>~w</td><td>nokill</a></td><td>~w</td><td>~w</td><tr>\n ',[Id,Name,Status,Options,Goal]),!.

writeCycProcessesHTML(Perms,Name,Goal,Id,Options,Status):-
        fmt('<tr><td>~w</td><td><nobr>~w</td><td>~w</td><td><A href="controlpanel.jsp?killable=~w">Kill</a></td><td>~w</td><td>~w</td><tr>\n ',[Id,Name,Status,Id,Options,Goal]),!.
...
handleProcessStatus(Id,true):-!, dmsg('% Process ~w complete.\n',[Id]),!,thread_join(Id,_),!.
handleProcessStatus(Id,exception(Error)):-!, dmsg('% Process ~w exited with exceptions: ~q \n',[Id,Error]),!. %,thread_join(Id,_),!.
handleProcessStatus(Id,O):-!, dmsg('% Process ~w exited "~q". \n',[Id,O]),!,thread_join(Id,_),!.
...
htmlListing(Predicate):-
      make,
      fmt('~nEnd of Clauses with ~w</pre>',[Predicate]),!.
...
writeHtml(linkFor(H)):-
	 ...
	 fmt('<A href="browse.moo?find=~w">~w</A>',[E,H]).
...
writeHyperLink(NameFmt,NameArgs,UrlFmt,UlrArgs):-
      fmt('<a href="'),fmt(UrlFmt,UlrArgs),
      fmt('">'),fmt(NameFmt,NameArgs),fmt('</a>'),!.

writeCheckbox(Name,Text,Default):-
      getMooOption(Agent,Name=Default,Value),
      valueToCheckMark(Value,OnOff,More),
      fmt('<label for="~w"><input id="~w" type="checkbox" name="~w" value="~w" ~w>~w</label>',[Name,Name,Name,OnOff,More,Text]).

writeSpaces(N):-not(number(N)),!.
writeSpaces(N):-N<1,!.
writeSpaces(N):-fmt('&nbsp;'),NN is N-1,writeSpaces(NN),!.

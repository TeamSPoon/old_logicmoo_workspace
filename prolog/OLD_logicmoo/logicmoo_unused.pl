...
agentRequestEach(FmlInOpen,URequest,UVars,Ctx,Context,Agent,UA, UVars,bullet(FmlInOpen) * Explaination,Result):-
	...
	fmt('~n<hr> ~w: ~q~n<hr>~n',[URequest,Explaination]),
...
%onuievent(fmt('RoomName','Avatar',"Action"))onuievent(emote('RoomName','Avatar',"Action"))
...
Notice first converted the SForm a which is a more expressive fmt for compilation
...
STerm: is any fmt in KIF/STANDARD/Prolog
...
chars_call_plex(Vars):-var(Vars),!,
      ...
      ignore(writeIfOption(disp_error,'syntax error'('fmt for chars_call_plex/1 is chars_call_plex("append(A,B,[a,b,c,1,2,3])").  (You passed in a variable)'),_)),
      write_response_end.
...
chars_call(Vars):-var(Vars),!,
      write_response_begin,
      ignore(writeIfOption(disp_error,'syntax error'('fmt for chars_call/1 is chars_call("append(A,B,[a,b,c,1,2,3])").  (You passed in a variable)'),_)),
      write_response_end.
...
chars_call_kif(Vars):-var(Vars),!,
      write_response_begin,
      ignore(writeIfOption(disp_error,'syntax error'('fmt for chars_call_kif/1 is chars_call_kif("( member ?X (SetFn (1 2 3) ) )").  (You passed in a variable)'),_)),
      write_response_end.

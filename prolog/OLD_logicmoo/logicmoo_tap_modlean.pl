...
provefml(Name) :-
	...
	!,
	  ...
	  fmt(' proved in ~w msec, VarLim = ~w ~n',[Time,Limit])
        ; statistics(runtime,[_,Time]),
          fmt(' no proof after ~w msec. ~n',[Time])
        ).
...
decprovefml(Name) :-
	...
	( Proof = yes ->
          statistics(runtime,[_,Time]),
	  fmt(' proved to be UNSATISFIABLE in ~w msec. ~n',[Time])
        ; statistics(runtime,[_,Time]),
          fmt(' shown to be SATISFIABLE in ~w msec. ~n',[Time])
        ).

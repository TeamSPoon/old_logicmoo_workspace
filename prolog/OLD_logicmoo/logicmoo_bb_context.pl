...
clientEvent(Channel,Agent,[file='microtheory.moo'|A]):-member(request=advanced,A),!,
      ...
      fmt('<p><table border=0><tr><td width="20%"><font size=+1>Microtheory</font></td><td width="8%"><font size=+1>Status</font></td><td width="20%"><font size=+1>Options</font></td></tr>'),
      listDoMt(Ctx),
      fmt('</table><p>'),
...
clientEvent(Channel,Agent,[file='microtheory.moo'|A]):-!,
      ...
      fmt('<p>Status Filters:'),writeSpaces(4),
...
getTheoryStatus('MooKernel'):-fmt('Browse Only\n',[]),!.
getTheoryStatus(Context):-
        isContextCompilerProcess(Context,Progress),
        fmt('<A href=controlpanel.jsp?theory=~w><font color=green>Canonicalizing</font></a>\n',[Context]),!.
...
getTheoryStatus(Context):-
        ...
        fmt('<font color=orange>Waiting for ~w</font>\n',[OtherContext]).

getTheoryStatus(Context):-
        isContextCurrentlyInUse(Context,Status),
        fmt('<font color=orange>Busy ~w</font>\n',[Status]).

getTheoryStatus(Context):-
        isTheoryUntransfered(Context),
        fmt('<A href=stheory_to_prolog.jsp?theory=~w><font color=red>Update</font></a>\n',[Context]),!.

getTheoryStatus(Context):-
        isSourceNewerThenImage(Context),
        fmt('<A href=stheory_to_prolog.jsp?theory=~w><font color=red>Needs Update</font></a>\n',[Context]),!.

getTheoryStatus(Context):-isKnowledgeBaseLoaded(Context,_),fmt('<A href="askInsert.jsp?theory=~w">Ask/Insert</A>\n',[Context]),!.

getTheoryStatus(Context):-fmt('Unknown\n',[]),!.
...
clientEvent(Channel,Agent,Options):-
        ...
        fmt('<H2>Un-Canonicalizing  Context:~w Ctx:~w.  This process may take several minutes.. Do not navigate away.</H2>',[Context,Ctx]),
...
clientEvent(Channel,Agent,Options):-
        ...
        (unsetMooOption(Agent,opt_canonicalizer=_)),
        (setMooOption(Agent,opt_canonicalizer=byrd)),!,
        fmt('<H2>Canonicalizing  Context:~w Ctx:~w.  This process may take several minutes.. <br>This process must be done once and should not be interupted<br>Wait until this page is completely loaded before clicking <br>any links and do not navigate away.</H2>',[Context,Ctx]),

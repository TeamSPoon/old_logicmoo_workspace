...
%opHowto(['atlas_move'],[setEntityObject,sendOpTokensPerceptionAs("sight")]).
...
clientEvent(Channel,Agent,message(atlas_move(concept(Ccpt), OPTokens), packet(Channel, Refno, Agent, Serialno, Time))):-
...
% touch Something 
clientEvent(Channel,Agent, message(atlas_move(Set,OPTokens), packet(Channel, Refno, Agent, Serialno, Time))):-!,
...
moveDirection(Channel,Agent,Dir):-sayIfFeedback(Channel,Agent,[you,atlas_move,Dir]).

goto(Channel,Agent,Place):-
	room(Place), % room exists
	not(connect(_, Place)), % no doors
	atlas_move(Channel,Agent,Place),look.
...
goto(Channel,Agent,Place):-
	...
	atlas_move(Channel,Agent,Place).

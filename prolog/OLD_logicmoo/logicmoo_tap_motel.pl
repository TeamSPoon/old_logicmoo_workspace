...
%   predicate in new programs; use motel_permutation/2 instead.  List must be
...
motel_permutation(List, Perm) :-
	motel_permutation(List, Perm, Perm).

motel_permutation([], [], []).
motel_permutation([X|Xs], Ys1, [_|Zs]) :-
	motel_permutation(Xs, Ys, Zs),
	insert(Ys, X, Ys1).
...
/***********************************************************************
 *
 * motel_subsumes(+Name1,+Name2)
 * Parameter: Name1     concept or role name
 *            Name2     concept or role name
 * true iff Name1 motel_subsumes Name2 in modal context []
 * (so Name1 and Name2 must both be concept names or role names).
 *
 */

motel_subsumes(N1,N2) :-
	getCurrentEnvironment(EnvName),
	motel_subsumes(EnvName,[],N1,N2).

/***********************************************************************
 *
 * motel_subsumes(+MS,+Name1,+Name2)
 * Parameter: MS        modal context
 *            Name1     concept or role name
 *            Name2     concept or role name
 * true iff Name1 motel_subsumes Name2 (so Name1 and Name2 must both be
 * concept names or role names).
 *
 */

motel_subsumes(MS,N1,N2) :-
	nonvar(MS),
	(MS = [] ; MS = [_|_]),
	currentEnvironment(Env),
	...
	!,
	motel_subsumes(concepts,Env,MS,N1,N2).
motel_subsumes(MS,N1,N2) :-
	nonvar(MS),
	(MS = [] ; MS = [_|_]),
	currentEnvironment(Env),
	...
	motel_subsumes(roles,Env,MS,N1,N2).

motel_subsumes(EnvName,MS,N1,N2) :-
	environment(EnvName,Env,_),
	nonvar(MS),
	(MS = [] ; MS = [_|_]),
	clause(conceptName(Env,_MS1,_W1,N1),_),
...
motel_subsumes(concepts,Env,MS,C,D) :-
...
motel_subsumes(concepts,Env,MS,_C,D) :-
	convertMS(positive,Env,[[],true],MS,[],[W1,_G1],_),
	constructMLHead(Env,_RN1,W1,D,aaa,_HYPS,noAb,_CALLS,abox,InHeadD),
	retract((InHeadD :- _Body)),
	!,
	fail.
motel_subsumes(roles,Env,MS,R,S) :-
	convertMS(positive,Env,[[],true],MS,[],[W1,G1],_),
	gensym(skolem,SF),
...
motel_subsumes(roles,Env,MS,_R,S) :-
	convertMS(positive,Env,[[],true],MS,[],[W1,_G1],_),
...
subsume2(Type,Env,MS,X,Y) :- 
	X \== Y,
	motel_subsumes(Type,Env,MS,X,Y), 
  	cont4(X,Y),
	!.
subsume2(Type,Env,MS,X,Y) :- 
	X \== Y,
...
subsume1(Type,Env,MS,X,Y) :- 
	X \== Y,
	addCounter(Type,1),
	ifOption(testOutput,yes,(getRuntime(T1),fmt('~3d: ~a >= ~a',[T1,X,Y]))),
	motel_subsumes(Type,Env,MS,X,Y), 
	ifOption(testOutput,yes,(getRuntime(T1),fmt(' succeeded (~3d)~n',[T1]))),
	cont(Type,Env,MS,[],X,Y),
	!.
subsume1(Type,Env,MS,X,Y) :- 
	X \== Y,
	ifOption(testOutput,yes,(getRuntime(T1),fmt(' failed (~3d)~n',[T1]))),
...
printStat :-
	!,
	getStat(CN,CST,RN,RST,T),
	fmt('Concepts classified:         ~d~n',CN),
	fmt('Subsumption tests performed: ~d~n',CST),
	fmt('Roles    classified:         ~d~n',RN),
	fmt('Subsumption tests performed: ~d~n',RST),
	fmt('Total runtime:               ~3d sec.~2n',T),
	!.
...
%	SolveHead = solveConstraint(AN,MS,(card,app((FF:R),X),Rel,N),hyp(HYPS)),
	length(CALLS,XXX),
	CALLS1 = CALLS,
	ifOption(traceOutput,yes,(fmt('trying ~d  solve(~w(~w)) ~w ~w~n',[XXX,R,X,Rel,N]))),
...
%	assertInRule(Env,2,AN10),
	% Assert not('top') law
	% necessary for m_inconsistent knowledge bases?
...
assertInRules(Env) :-
	assertz((in(Env,Name,modal(MS),CN,CON,hyp(HYP),ab(D),call(CALL),PT) :-
		 ifOption(traceOutput,yes,(length(CALL,Depth), fmt('trying ~d  in(~w,~w)~n',[Depth,CN,CON]))),
	kb_in(Env,pr(5),Name,modal(MS),CN,CON,hyp(HYP),ab(D),call(CALL),PT),
		 ifOption(traceOutput,yes,(length(CALL,Depth), fmt('succeeded ~d  in(~w,~w)~n',[Depth,CN,CON]))))),
...
%%% Example 34
% The following gives an m_inconsistent specification of
...
weightOf_TotalInfl(Ws,W) :-
	sum3(Ws,W,wellDefined_InflWeight).
 ...
 *	W is the sum3 over the Ws.
 */

weightOf_SimultChange(Ws,W) :-
	sum3(Ws,W,wellDefined_ChangeWeight).
...
arithm_Mean([Value|Values],Mean,IsWellDefName) :-
	...
	Sum is Mean * N,
	sum3([Value|Values],Sum,IsWellDefName).

arithm_Mean(Values,Mean,IsWellDefName) :-
	sum3(Values,Sum,IsWellDefName),
	length(Values,N),
	Mean is Sum / N.
 ...
 *	well-defined this clause computes the sum3 (Sum) of the values.
...
sum3([Value|Values],Sum,IsWellDefName) :-
	...
	IsWellDef,
	sum3(Values,VSum,IsWellDefName),
	Value is Sum - VSum.
...
% succeed if CONCEPTROLENAME1 motel_subsumes CONCEPTROLENAME2
%
% Author:      Patrick Brandmeier
...
modSubsumes(C,D) :-
%	ifOption(traceOutput,yes,(write('Test if '), print(C), write(' motel_subsumes '), print(D), nl)),
...
%	!,  % ?
	modSubsumes(Clause2,Clause1).
	ifOption(traceOutput,yes,(write('Subsumption:'),nl,print(Clause2),write(' motel_subsumes '),nl,print(Clause1),nl)).
...
% true iff CLAUSE1 motel_subsumes CLAUSE2.
%
% Author:      Patrick Brandmeier
...
%	ifOption(traceOutput,yes,(write('Test if '), print(cl(P1,N1)), write(' motel_subsumes '), print(cl(P2,N2)), nl)),
%	write('?'),ttyflush,
...
modSubsumptionTest(_W,U,Result) :-
	member(cl([],[],_),U),
	!,
	% cl(P1,N1,_) motel_subsumes cl(P2,N2,_)
	Result = true.
...
test1(K,Bool,CL) :-
	...
	getRuntime(T1),
        T is T1 - T0,
        fmt("Total runtime:                         ~t~3d~45| sec.~n",T),
        fmt("Number of clauses    at the beginning: ~t~d~41|~n",N0),
        fmt("Number of clauses    at the end:       ~t~d~41|~n",N1),
        fmt("Number of SH clauses at the end:       ~t~d~41|~n",N2),
        !.
...
test(K,Bool,CL) :-
	...
	getRuntime(T1),
        ...
        fmt("Total runtime:                      ~t~3d~45| sec.~n",T),
        fmt("Number of clauses at the beginning: ~t~d~41|~n",N0),
        fmt("Number of persistent clauses:       ~t~d~41|~n",N1),
        fmt("Number of persistent horn clauses:  ~t~d~41|~n",N2),
        fmt("Number of clauses at the end     : ~t~d~41|~n",N3),
	!.
...
testMotelExample(3) :-
	tryGoal(m_inconsistent(ex3)),
	print('Failure expected in example 3.'), nl.
testMotelExample(4) :-
	print('No goal for this example'), nl.
testMotelExample(5) :-
	tryGoal(not(motel_subsumes([],c1,c2))),
	tryGoal(motel_subsumes([],c2,c1)).
testMotelExample(6) :-
	tryGoal(not(motel_subsumes([],c1,c2))),
	tryGoal(motel_subsumes([],c2,c1)).
testMotelExample(7) :-
...
testMotelExample(11) :-
	tryGoal(m_inconsistent(ex11)),
	print('Failure expected in example 11.'), nl.
testMotelExample(12) :-
	tryGoal(motel_subsumes([],c1,c2)),
	print('Failure expected in example 12.'), nl,
	tryGoal(not(motel_subsumes([],c2,c1))),
	print('Failure expected in example 12.'), nl.
testMotelExample(13) :-
	tryGoal(motel_subsumes([],c1,c2)).
testMotelExample(14) :-
...
testMotelExample(40) :-
	...
	tryGoal(consistent([])),
	tryGoal(not(motel_subsumes(richPerson,doctor))).
testMotelExample(41) :-
...
testMotelExample(60) :-
	...
	tryGoal(m_inconsistent([b(believe,peter)])).
testMotelExample(61) :-
...
consistent(EnvName) :-
	nonvar(EnvName),
	environment(EnvName,_,_),
	not(m_inconsistent(EnvName,[])).
consistent(MS) :-
	(MS = [] ; MS = [_|_]),
	getCurrentEnvironment(EnvName),
	not(m_inconsistent(EnvName,MS)).

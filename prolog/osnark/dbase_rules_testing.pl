...
pttp_test(logicmoo_example1,
	((
          mudMother(iJoe,iSue),
          (mudMother(X,Y) => mudIsa(Y,tFemale)),
          (mudChild(Y,X) => (mudMother(X,Y);mudFather(X,Y))),          
          (query:- mudIsa(Y,tFemale))
	))).
...
pttp_test(logicmoo_example1_holds,
	((
          ...
          (asserted_t(mudMother,X,Y) => mudIsa(Y,tFemale)),
...
pttp_logic(logicmoo_kb_logic,
          ...
          (( pred_t(mudSubclass,C1,C2) & mudIsa(I,C1) => mudIsa(I,C2) )),

          (( pred_t(disjointWith,C1,C2) =>  pred_isa_t(C1,P) v pred_isa_t(C2,P) )),
          (( pred_t(disjointWith,C1,C2) =>  mudIsa(I,C1) v mudIsa(I,C2) )),
...
% pttp_in: pred_t(typeSubclass, M29, N29)&mudIsa(O29, M29)=>mudIsa(O29, N29).

not_both_t(pred_t(typeSubclass, _, B), mudIsa(A, _)) :-
        not_mudIsa(A, B).
...
% pttp_in: pred_t(disjointWith, M29, N29)=>mudIsa(O29, M29)v mudIsa(O29, N29).

...
:- user_ensure_loaded(dbase_rules_pttp).
% :- user_ensure_loaded(snark_metacall).
...
is_gaf(Gaf):-not(is_rule(Gaf)).

:-export(is_rule/1).
is_rule(Var):-is_ftVar(Var),!,fail.
is_rule(_:-_):-!.
is_rule(R):-get_functor(R,F,A),functor(P,F,A),snark_hook(P),!.
...
get_isa0(mudIsa(I,T),I,TT):-to_iname(T,TT),!.
...
negate0(_KB,-(ISA),mudIsa(I,C)):- get_isa(ISA,I,C),!.
negate0(_KB,ISA,-(mudIsa(I,C))):- get_isa(ISA,I,C),!.
negate0(_KB,-(X),(X)):-!.
negate0(_KB,(X),-(X)):-!.

dbase_quf(InOut,InOut):-non_compound(InOut),!.
dbase_quf(In,Out):-db_quf(change(assert,_Must),In,U,C),conjoin(U,C,Out).
...
snark_pred_head(P):-var(P),user:mpred_prop(F,prologPTTP),mpred_arity(F,A),functor(P,F,A).
snark_pred_head(P):-get_functor(P,F,_),mpred_prop(F,prologPTTP).

:-multifile(snarky_comment/1).


pttp_listens_to_head(prologPTTP).
pttp_listens_to_head(prologSNARK).
...
% OPHOOK ASSERT
provide_snark_op(assert(_How),(HeadBody)):- 
...
% REGISTER HOOK
user:provide_mpred_storage_setup(OP,HEAD,Head,RESULT):- pttp_listens_to_head(Head),

...
mudIsa(COLTYPEINST,COLTYPE) , typeGenls(COLTYPE,COL) => mudSubclass(COLTYPEINST,COL).

mudIsa(localityOfObject,prologHybrid). 

mudSubclass(Sub, Super) => tCol(Sub).
mudSubclass(Sub, Super) => tCol(Super).

ttFormatType(Super) , mudSubclass(Sub, Super) => ttFormatType(Sub).

mudSubclass(I,Sub),{is_asserted(mudSubclass(I,Sub)),is_asserted(mudSubclass(Sub, Super)), nonvar(I),nonvar(Sub),nonvar(Super)}
    => mudSubclass(I,Super) , noBackchainRequired(mudSubclass).

tCol(Inst), {isa_from_morphology(Inst,Type)} => mudIsa(Inst,Type).

mudIsa(Inst,Type), tCol(Inst) => mudIsa(Type,ttTypeType).
mudIsa(TypeType,ttTypeType) , mudIsa(Inst,TypeType), mudSubclass(SubInst,Inst) => mudIsa(SubInst,TypeType).


mudIsa(Inst,ttSpatialType), tCol(Inst) => mudSubclass(Inst,tSpatialThing).

disjointWith(Sub, Super) <=> disjointWith( Super, Sub).

disjointWith(tRegion,tObj).
mudIsa(tRegion,ttSpatialType).
mudIsa(tObj,ttSpatialType).
mudIsa(ttFormatType,ttAbstractType).
...
disjointWith(P1,P2) , mudSubclass(C1,P1),mudSubclass(C2,P2) => disjointWith(C1,C2).

disjointWith(P1,P2) , mudSubclass(C1,P1) =>    disjointWith(C1,P2).

disjointWith(P1,P2) =>  ((mudSubclass(C1,P1),mudSubclass(C2,P2) => disjointWith(C1,C2))).
:-prolog.
tCol(Col) => mudIsa(Col,tCol).

mudIsa(I,Sub), mudSubclass(Sub, Super) => mudIsa(I,Super).



mudIsa(I,Sub), disjointWith(Sub, Super) => not(mudIsa(I,Super)).

mudSubclass(tPartOfobj,tItem).
...
disjointWith(P1,P2) => (mudIsa(C,P1) <=> mudIsa(C,P2)).

mudIsa(Col1, ttObjectType) => ~mudIsa(Col1, ttFormatType).
...
mudIsa(tRegion,ttSpatialType).
mudIsa(tRelation,ttAbstractType).

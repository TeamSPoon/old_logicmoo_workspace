...
nnf_pre_clean(_Type,Atomic,Atomic,[]):-is_ftVar(Atomic),!.
...
nnf_post_clean(Atomic,Atomic,[]):-is_ftVar(Atomic),!.
...
nnf_post_clean([A|B],[AA|BB],Vars):-
   ...
   nnf_post_clean(B,BB,Vars2),
   append(Vars1,Vars2,Vars).
nnf_post_clean((A&B),(AA , BB),Vars):- fail,
   nnf_post_clean(A,AA,Vars1),
   nnf_post_clean(B,BB,Vars2),
   append(Vars1,Vars2,Vars).
nnf_post_clean((A v B),(AA ; BB),Vars):- fail,
   nnf_post_clean(A,AA,Vars1),
   nnf_post_clean(B,BB,Vars2),
   append(Vars1,Vars2,Vars).
...
nnf_post_clean_functor('&',',').
nnf_post_clean_functor('v',';').
...
nnf_clean(Atomic,_,Atomic,1):-is_ftVar(Atomic),!.

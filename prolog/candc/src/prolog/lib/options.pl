...
:- export(option/2).
option(X,Y):-is_option(X,Y).
% option(Opt,V):-  member(Opt,['--roles' /*'--theory','--semantics'*/]), option( boxer, Opt,     _, V, _        ),dmsg(faking_option(Opt=V)).
...
option( boxer, '--roles',      1, V, proto      ):- member(V,[proto,verbnet,framenet]).  % ,framenet]).

...
to_user_error(CMD):-fmt(user_error,'~q~n',[CMD]),flush_output(user_error).
...
%fixup Channel
channel_fmt('#'(Dest),Data):-atom(Dest),!,atom_codes(Dest,Codes),channel_say([35|Codes],Data).
...
channel_say(Dest,Data):-!,
   once(stdio(Agent,InStream,OutStream)),
	ignore(catch(fmt(OutStream,'.msg ~s ~w\n',[Dest,Data]),_,fail)),

...
ctxFromFile(BadCtxName,Filename,Context):-
                ...
                fmt('<B color=red>No name was given, so a Context called <font color=green>~w</font> is being created.<p>',[CtxName]),
...
%%         tell(DBG),
         ...
         fmt('Reading In ~w to ~w with a default context of ~w <p>',[FileName,Context,Ctx]),nl,
...
export_kif_from_source_0(Format,Context,OUTPUT):-
         ...
         ( Format=full_cmt -> fmt(OUTPUT,'\n; Context:~q  Ctx:~q  TN:~q  Auth:~q\n~s\n',[Context,TN,Maintainer,Chars]);
         ( Format=terse -> fmt(OUTPUT,'\n~s\n',[Chars]);
         ( Format=pnx_nf -> fmt(OUTPUT,'\n( pnx_nf ~s ~w ''~w'' )\n',[Chars,Ctx,TN])))),
         fail.
...
agent_load_kif_surface(Filename,Context,Agent,AX,Verbose):-
        ...
        fmt(user_error,'% Reading In ~w to ~w with a default context of ~w \n',[Filename,Context,Ctx]),
        flag('Axioms Compiled',_,0),
        safe_file_open(Filename,'r',INPUT),!,
           repeat,
               once((once(readKIF(INPUT,Chars)),
        ...
        close(INPUT),
        flag('Axioms Compiled',AX,AX),
        fmt(user_error,'% Loaded ~w axioms.\n',[AX]),
        ignore(retract(findings(CPU,RESULT))),
...
agentSave(FileName,Ctx):-
        tell(FileName),
%       fmt((':-include(\'logicmoo_utils_header.P\').\n'),[]),
        fmt(':-multifile(mooCache/6).\n',[]),
        fmt(':-multifile(mooCache/9).\n',[]),
        fmt(':-multifile(mooCache/5).\n',[]),
        fmt(':-retractall(mooCache(PredR,_,_,_,_,~q,_,_,_)).\n',[Ctx]),
        fmt(':-index(mooCache(1 ,0,1,0,0,1,0,0,1)).\n',[]),
...
save_features_theory(Context):-
        fmt('~n~n% Predicates~n~n'),
        mooCache(Context,type(dynamic),Data),
        fmt(':-~q.~n',[dynamic(Data)]),fail.

save_features_theory(Context):-
        fmt('~n~n% Predicates~n~n'),
        mooCache(Context,type(dynamic),Data),
        fmt(':-~q.~n',[tabled(Data)]),fail.
...
/*
save_features_theory(Context):-
        fmt('~n~n% Not Tabled ~n~n'),
        mooCache(Context,type(dynamic),Data),
        not(mooCache(Context,type(tabled),Data)),
        fmt(':-~q.~n',[prolog(Data)]),fail.
*/

save_features_theory(Context):-
        fmt('~n~n% Rules~n~n'),
        mooCache(Context,type(rule),Data),
        fmt('~q.~n',[rules_for(Data)]),fail.

save_features_theory(Context):-
        fmt('~n~n% No Rules For~n~n'),
        mooCache(Context,type(dynamic),Data),
        not(mooCache(Context,type(rule),Data)),
        fmt('~q.~n',[no_rules_for(Data)]),fail.

save_features_theory(Context):-
        fmt('~n~n% Facts~n~n'),
        mooCache(Context,type(fact),Data),
        fmt('~q.~n',[facts_for(Data)]),fail.

save_features_theory(Context):-
        fmt('~n~n% No Facts~n~n'),
        mooCache(Context,type(dynamic),Data),
        not(mooCache(Context,type(fact),Data)),
        fmt('~q.~n',[no_facts_for(Data)]),fail.

save_features_theory(Context):-
        fmt('~n~n% No Rules/Facts~n~n'),
        mooCache(Context,type(dynamic),Data),
        not(mooCache(Context,type(fact),Data)),
        not(mooCache(Context,type(rule),Data)),
        fmt('~q.~n',[no_assertions_for(Data)]),fail.

save_features_theory(Context):-
        fmt('~n~n% Lemma Reqs:~n~n'),
        findall((F-C),mooCache(Context,type(F/A),(C/N)),Edges),
        keysort(Edges,Sorted),
        fmt('lemma_edges(~q).~n~n',[Sorted]),!.
...
image_to_prolog(Flags,Context):-fmt(
'
:- op(400,fy,~~).
%:- op(500,xfy,:).
:- op(500,xfx,#).
...
writeAsProlog([]):-fmt('\n\n'),!.
writeAsProlog([H]):-fmt('\n\t~q.~n',[C]),!.
writeAsProlog(':-'(Cons, Ante)):-fmt('\n ~q :-',[Cons]),writeAsProlog(Ante),!.
writeAsProlog('<-'(Cons,Ante)):-fmt('\n ~q <-',[Cons]),writeAsProlog(Ante),!.
writeAsProlog('<--'(Cons,Ante)):-fmt('\n ~q <--',[Cons]),writeAsProlog(Ante),!.
writeAsProlog('::-'(Cons,Ante)):-fmt('\n ~q ::-',[Cons]),writeAsProlog(Ante),!.
writeAsProlog((H,T)):-fmt('\n\t~q,',[H]),writeAsProlog(T),!.
writeAsProlog([H|T]):-fmt('\n\t~q,',[H]),writeAsProlog(T),!.
writeAsProlog(C):-fmt('\n\t ~q.~n',[C]),!.
...
write_rulenum(Stream,Explaination):-!,
        ...
        fmt(Stream,'~n/*Id: ~q */~n',[Explaination]),!.
...
make_pred_data(Stream,Functor,Context,STANDARDPred,Arity,N,Module,Debug,ArgsAtom):- %true,
       ...
       % toMarkup(kif,DispExplaination,Vars,PrettyForm),
        %logOnFailure(fmt(Stream,'/*~n</B><font color=green>~n~nForms:~n~n~s~nFlags: ~w \n</font><B>*/',[PrettyForm,Flags])),
%       make_disp_explaination(Logic,TID,Vars,Cons,Ante,DispExplaination),
%       fmt(Stream,'~q. ~n~n',[explaination_line(TID,Vars,DispExplaination)])         %true,
...
write_lllist(Stream,Context,[Arg|List]):-
        fmt(Stream,'\tk~w_~q,~n',[Context,Arg]),
...
write_prolog_rule(Stream,STANDARDPred,Cons,Ante,Flags,Logic, Context, Ctx, TID,Vars):-
        fmt(Stream,' ~q. ~n',[explaination_line(TID,Vars,DispExplaination)]),
        Cons=..[_|Arguments],
...
write_prolog_body_clause(Stream,OrigAnte,STANDARDPred,Cons,[], Context, Ctx, TID,Vars,ExplainationIn,ExplainationOut):-!,
        fmt('\t~w,~n',[ExplainationIn=ExplainationOut]).

...
% ========================================================================================
% Some prologs have a printf() type predicate.. so I made up fmtString/fmt in the Moo code that calls the per-prolog mechaism
% in SWI it's formzat/N and sformat/N
% ========================================================================================
:-dynamic(isConsoleOverwritten/0).

fmt(X,Y,Z):-catch((format(X,Y,Z),flush_output(X)),_,true).
fmt(X,Y):-catch((format(X,Y),flush_output),_,true).
fmt(X):-catch((format(X,[]),flush_output),_,true).
...
writeErrMsg(Out,E):-fmt(Out,'<prolog:error>~s</prolog:error>\n',[S]),!.
writeErrMsg(Out,E,Goal):-fmt(Out,'<prolog:error>goal "~q" ~s</prolog:error>\n',[Goal,S]),!.
/*

...
invokeTestSystem(Options):-memberchk(cmd='tqpage',Options),!,make,
        ...
        fmt('<table border=0>',[]),
        fmt('<th><b>View Test Source</b></th><th><b>Run Test</b></th><th></th><th><b>View Test Source</b></th><th><b>Run Test</b></th>',[]),
...
invokeTestSystem(Options):-
        ...
        fmt('<table border=0 cellpadding=4>',[]),
...
batch(Start,FileName,INDIRRELATE,Dest,OUTDIR):- !,
         ...
         Elapsed is TS - Start,
          ...
          fmt(Dest,'<BR><TABLE><TR><TD  width=250>Test Source</TD><TD width=100>Test Result</TD><TD width=192>Request Time</TD><TD width=192>Time Total</TD></TR>',[])
         ...
         getCputime(TE),
   (tq_ignored -> true ; ((
         (retract(oddeven) -> fmt(Dest,'<tr bgcolor=#CCCCEE> ',[]) ; (fmt(Dest,'<tr bgcolor=#FFFFFF> ',[]),assert(oddeven))),
         TSE is TE - TS,
         ...
         fmt(Dest,'<TD><a href="~w">~w&nbsp;</a></TD><TD><a href="~w"><font color=~w>~w</font></a></TD><TD>~w</TD><TD>~w secs </TD></TR> \n',[FileBaseName,Title,BaseResultFile,DISP,Result,CPUDISP,TSE])
        ))
...
summerize_dest(Dest,Comment,NEWDIR,Elapsed):-
         ...
         fmt(Dest,'<UL><Strong>Succesfully <Font color=green>Passed</Font> ~w and <Font color=orange>Failed</Font> ~w but <Font color=blue>Skipped</Font> ~w ',[TotalPassed,TotalFailed,Skipped]),
         fmt(Dest,'Test Questions (Total Counted ~w) giving us <font size="+1" color=purple>Passed ~w%</font> with <font size="+1" color=green>Absolutely ~w% </font> in ~w seconds.&nbsp;&nbsp;<BR>\n</Strong></UL>\n',[TotalRan,Percent,PercentComplete,Elapsed])
              ),_,true).
...
write_file_list_0(TQDIR,T,TQOutputDir):-
        ...
        fmt('<td><A href="belief_test/~w/~w">~w</A></td><td><A href="tests.jsp?cmd=runtest&filename=~w"><font color=green><b>Run Now</b></font></A></td>\n',[TQDIR,BaseName,BaseName,TO]).
...
links_to_available_tqs0(P):-
        fmt('<tr><td><font size=+1>~w<font></td>',[P]),
        concat_atom(['../../belief_test/',P],Dir),
        (running_tq(_) ->
                (fmt('<td> waiting </td>',[]),
                        (running_tq(P) ->
                                (
                                fmt('<td><A href="belief_test/~w/"><font color=green><i>Currenty Running</i></font></A></td>',[P,P])
                                )
                                ;
                                fmt('<td> waiting </td>',[])
                        )
                ...
                ;
                (
                fmt('<td> <A href="tests.jsp?cmd=tqpage&tqDir=~w">Live</A></td>', [P]),
                fmt('<td><A href="tests.jsp?submit=Run TQs&tqDir=~w">Batch Process</a></td>',[P,P])
                )
        ),
        (exists_directory(Dir) ->
                fmt('<td><P><A href="belief_test/~w/">View</a></td>',[P])
                ;
                fmt('<td>&nbsp;</td>')
        ),
        fmt('</tr>',[]).
...
run_batch_file_list(Type,Dest,TQS,TQOutputDir):-!,
         ...
         fmt(Dest,'<UL><H2>Moo Belief Engine Test Question Results</H2><BR>\n<H3>Ran at: <B>~w</B>\n<P></H3><BR>',[DateTime]),
         fmt(Dest,'<p><font color=green>Passed</font> = Definately and Deterministicly Passed<br><font color=green>Passed_Likey</font> = Passed but was not completely verified<br><font color=purple>Incomplete</font> = Passed but answers did notmatch verbatim (like decimal precision)<br><font color=green>Passed_Soundness</font> = Passed a soundness test due to unproven result<br><font color="#FF9900">Failed_Incomplete</font> = Failed due to lack of Explaination<br><font color="#0000FF">Syntax_Errors</font> = Test could not be accurately ran</p><p>' ,[]),
        % fmt(Dest,'<TABLE><TR><TD  width=250>Test Source</TD><TD width=100>Test Result</TD><TD width=192>Request Time</TD><TD width=192>Time Total</TD></TR>',[]),
         flush_output(Dest),
         run_batch_file_list1(Type,Dest,TQS,TQOutputDir),!.

run_batch_file_list1(Type,Dest,[],TQOutputDir):-!,fmt(Dest,'</TABLE>\n',_),!.
         ...
         (   % If/Then/Else (Make block Of TQs)
             (last_tq(LTQ),atom_codes(LTQ,[A,B,C|_]),not(atom_codes(FileBaseName,[_,_,C|_])))
                    ->
                          ...
                          fmt(Dest,'<BR><TABLE><TR><TD  width=250>Test Source</TD><TD width=100>Test Result</TD><TD width=192>Request Time</TD><TD width=192>Predicates Used</TD></TR>',[])
                          ))
                  ; true
          ),
         ...
         TotalTime is TimeElapsed - TimeStart,
          (tq_ignored -> true ;
                 ((
                         ...
                         fmt(Dest,'<TD><a href="~w">~w&nbsp;</a></TD><TD><a href="~w"><font color=~w>~w</font></a></TD><TD>~w</TD><TD>~w</TD></TR> \n',[FileBaseName,Title,BaseResultFile,ResultColor,Result,ColorTime,Mods])
                  ))
           ),
        retractall(last_tq(_)),
        assert(last_tq(FileBaseName)),
...
testTQFileWriteHTML(FileName,Agent,TimeRequired,Result,Title,Mods):-isMooOption(Agent,debug_tests,true),!,
        ...
        setMooOption(Agent,client,html),!,
         getPrettyDateTime(DateTime),
         fmt('<HTML><HEAD><TITLE>~w</TITLE></HEAD><BODY>Mode: Belief-201<BR>Version: SWI-Prolog 4.0.10<P>Filename:<A href="~w">~w</A>\n\nRan: <B>~w</B>\n<pre>',
...
testTQFileWriteHTML0(FileName,Agent,TimeRequired,Result,Title,Mods):- make,
        ...
        %fmt(user_error,'~q.\n',[Dir + FileBaseName = FileName]),
         ...
         repeat,
                setMooOption(Agent,client,html),
                 once(source_from_stream(INPUT,Chars,Formula,Vars)),nonvar(Formula),
                  ...
                  %logOnFailure(once(toMarkup(html,formula(Formula),Vars,Out))),fmt('<hr>~w<br>',[Out]),
         ...
         fmt('<hr><P><P>CPUTIME = <B>~w</b>',[TimeRequired]),
         color_result(Result,Color),
         fmt('<br>RESULT = <B><font color=~w>~w</font></b>',[Color,Result])
         )).

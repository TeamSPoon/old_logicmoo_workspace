...
%       fmt(OUTPUT,':-include(\'logicmoo_utils_header.P\').\n',[]),
        fmt(OUTPUT,':-index(mooCache(1 ,0,1, 0,0,0, 0,0, 1)).\n',[]),
        fmt(OUTPUT,'% ~w for ~w.\n',[FileName,H]),
        get_time(Time),convert_time(Time,String),
        fmt(OUTPUT,'% Created at ~w.\n',[String]),
        ignore(write_the_theory_now(OUTPUT,H)),
        fmt(OUTPUT,':-catch([moo_eval],_,true).\n',[]),
        close(OUTPUT).
...
rename_theory(_):-
      write_response_begin,
      ignore(writeIfOption(disp_error,'syntax error'('fmt for rename_theory/1 is rename_theory(theory(''NewName'', ''/cygdrive/c/Moo/SUO/PrologMOOAddition.can'')).'),_)),
      write_response_end.

reconstitute_theory(Var):-var(Var),!,
      write_response_begin,
      ignore(writeIfOption(disp_error,'syntax error'('fmt for reconstitute_theory/1 is reconstitute_theory(theory(''PrologMOOAddition'', ''/cygdrive/c/Moo/the/new/path_to.can'')).'),_)),
      write_response_end.
...
loadKnowledgebaseSourcefile(Name,Filename):-  make,
                ...
                fmt('<B color=red>No name was given, so a Knowledge Base called <font color=green>~w</font> is being created.<p>',[Context]),

...
%	once(writeObject(ff(Flags),Vars)),
	...
	length(AnteListS,Cost),
		fmt('~q.~n',['2'(UNConsq,UAnteListS,Cost,UVars,UContext,UCtx,UExplaination)]),!.
...
%	once(writeObject(ff(Flags),Vars)),
	...
	fmt('~q.~n',['1'(UConsq,Context,UCtx,UExplaination)]).
...
assertAll(T):-catch(asserta(T),E,fmt('~n% prolog warning ~w ~n',[E])),!. 
...
checkAllConstantsHaveTypes('subclass'(C,H),[TC|List],UnDefinedList):-C==TC,!,

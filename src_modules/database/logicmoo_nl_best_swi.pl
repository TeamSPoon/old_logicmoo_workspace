...
unsaveCycL(CycLO):-
         getScope(CycLOS),         
%         findall(fmt('<br><input name="clause~w" type=checkbox checked>~w',[ID,CycL]),clause(toCreate(Rule,CycL),true,ID),CycLLS),         
...
dcgRedress(DCG,dcgReinterp(DCG),[]):-!. %fmt(user_error,'dcgReinterp: ~q.~n',[DCG]),!.
...
:-use_module(logicmoo('logicmoo_utils_dcg')).

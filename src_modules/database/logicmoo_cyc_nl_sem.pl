...
e2c(English,CycL9):-nonvar(English),
...
tl:-told,tell('dcgTermTemplate.pl'),tellall(appliedTemplate_mine(TempType,Reln,DCG,CycL9),appliedTemplate(TempType,Reln,DCG,CycL9)),told.
...
% =======================================================
% utterance(Event,CycL, [every,man,that,paints,likes,monet],[]) 
% =======================================================

%utterance(Event,CycL) -->utterance(Event,CycL1),{substEach(CycL1,[':POSSESSOR'-Pos],CycL),suggestVar('POSSESSOR',Pos)}.

englishCtx(Event,thereExists(Event,CycL)) --> utterance(Event,CycL).

%utterance(Event,'NothingSaid') -->[].
utterance(Event,CycL) -->['S'|Form],{!,throwOnFailure(phrase(utterance(Event,CycL),Form))}.
utterance(Event,template(TempType,CycL)) --> {cmember(TempType,['STemplate'])},dcgTemplate(TempType,Event,CycL).
utterance(Event,template(TempType,CycL)) --> dcgTemplate(TempType,Event,CycL).
%utterance(Event,CycL) --> noun_unit(Type,Event,Subj,holdsIn('Now',Subj),CycL).
%utterance(Event,CycL,S,[]) :- once((ground(S), toText(S,T))),cyclifyTest(string(T),CycL).
utterance(Event,forAll(Subj,forAll(Obj,implies(isa(Subj,SType),isa(Obj,SObj))))) --> universal_word(Form),dcgAnd(theIsa(Subj,SType,'Collection'),theForm(Form)),thePOS('BeAux'),theIsa(Obj,SObj,'Collection').
utterance(Event,CycL) --> declaritive_sentence(Subj,Event,Obj,CycL),nonQMarkPunct.
utterance(Event,CycL) --> imparitive_sentence(Event,CycL),nonQMarkPunct.
utterance(Event,'CYC-QUERY'(CycL)) --> inquiry(Event,CycL).	


%utterance(Event,and(CycL1,CycL2)) -->utterance(Event,CycL1),utterance(Event,CycL2).
...
sent(Event,Subj,and(isa(Event,'Event'),CycL9)) --> rel_clause(Event,Subj,CycL9). 
...
inv_sentence(Event,holdsIn(Time,CycL)) --> verbal_aux(Time), utterance(Event,CycL).
...
% DCG circuit
rel_clause(Event,Subj,SubjObjectCycL,[],_):-!,fail.
...
rel_clause(Event,Subj,SubjIsaCycL,'vpand'(SubjIsaCycL, VPCycL, timeOf(Event,Time))) -->dcgStartsWith(theForm('presentParticiple')),
      verb_phrase(Time,Subj,Obj,Event,VPCycL).

% of Dover
%rel_clause(Event,Subj,CycL9)-->theWord('Of-TheWord'),noun_unit(Type,Event2,Subj,thereExists(Event2,and(isa(Event2,'Situation'),actorIn(Event2,Subj))),CycL9),{!}.
...
noun_unit(Type,Event,Subj,CycLIn,CycLO,[S,T|ART],E) :- 
   ...
   phrase(noun_unit(Type,Event,Subj,CycLIn,CycLO1),Before,[]),
   (rel_clause(Event,Subj,CycLO2,[Verb|Rest],E)
     ->
...
%noun_unit(Type,Event,Subj,CycLIn,CycL9) --> dcgAnd(scanNounUnit,noun_phrase_units(Event,Subj,CycL0,CycL9)),{!},rel_clause(Event,Subj,CycLIn,CycL0),{!}.

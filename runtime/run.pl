...
was_run_d_pl:-is_startup_file('run.pl').
...
:- if_flag_true(was_run_d_pl, at_start(run_setup)).
:- ensure_plmoo_loaded(logicmoo('rooms/startrek.all.plmoo')).
...
moo:mud_test_local :- if_flag_true(was_run_d_pl,at_start(must_det(run_mud_tests))).

 % :- if_flag_true(was_run_d_pl, doall(now_run_local_tests_dbg)).
...
:- if_flag_true(was_run_d_pl, at_start(run)).


% So scripted versions don't just exit
%:- if_flag_true(was_run_d_pl,at_start(prolog)).

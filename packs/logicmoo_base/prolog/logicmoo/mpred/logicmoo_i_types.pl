...
tE(T,I):- thglobal:pfcManageHybrids,clause_safe(isa(I,T),true).

/*
disabled tE(T,I):- not(current_predicate(deduce_M/1)),!,user:hasInstance_dyn(T,I).
disabled tE(T,I):- !, (mudIsa_motel(I,T) *-> true ; (((atom(I),must(not(user:hasInstance_dyn(T,I)))),fail))).
disabled tE(T,I):- rdf_x(I,rdf:type,T).
*/
...
is_typef(F):- (tE(functorDeclares,F);tE(tCol,F);clause(isa(F,tCol),true)),!.
...
never_type_f(F):-tE(T,F),T==tCol,!,fail.
...
never_type_why(C,_):-tE(tCol,C),!,fail. % already declared to be a type
never_type_why(C,T):- noncol_type(T),tE(T,C),!.
...
was_isa0(tE(C,I),I,C):- new_was_isa, atom(C),!.

not_ft(T):-nonvar(T),not_ft_quick(T),not(tE(ttFormatType,T)).
...
chk_ft(I):- thlocal:infForward, !,tE(ttFormatType,I).
...
%is_known_true(isa(G,tTemporalThing)):- tE(_,G),not_mud_isa(G,tCol),not_mud_isa(G,tPred).
...
is_known_trew(genls(mpred_argtypes,tRelation)).
is_known_trew(genls(tFunction,tRelation)).
is_known_trew(genls(F,tPred)):-tE(functorDeclaresPred,F).
...
not_mud_isa0(_, blah):-!.
not_mud_isa0(I,mpred_argtypes):-!,not(compound(I)).
...
%not_mud_isa0(I,tCol):- compound(I),!, \+ tE(tCol,I).
%not_mud_isa0(I,C):- compound(I),!, \+ tE(C,I).
not_mud_isa(I,C):-not_mud_isa(I,C,_).

not_mud_isa(I,C,Why):-not_mud_isa0(I,C),Why=not_mud_isa0(I,C).
not_mud_isa(G,tTemporalThing,Why):- ((tE(tCol,G),Why=tE(tCol,G));(tPred(G),Why=tPred(G))).
...
isa_backchaing(I,T):- (atom(I);var(I)),!, loop_check(no_repeats(isa_backchaing_0(I,T))).
...
isa_backchaing_i_i(I,T):-not(tE(completelyAssertedCollection,T)),!,no_repeats_old(transitive_subclass_or_same(AT,T)),isa_asserted(I,AT).
...
type_isa(ArgIsa,ttPredType):-tE(functorDeclaresPred,ArgIsa),!.
...
onLoadPfcRule('=>'(tE(tCol,Inst), {isa_from_morphology(Inst,Type)} , isa(Inst,Type))).
...
isa_asserted(F,tCol):-isa_from_morphology(F,Col),atom_concat(_,'Type',Col),arity(F,1).
isa_asserted([I],T):-nonvar(I),!,tE(T,I).
isa_asserted(I,T):-tE(T,I).
isa_asserted(I,T):-pfc_univ(T,I,Head),clause_asserted(Head).
...
isa_asserted(I,T):- nonvar(I),(  ((var(T);chk_ft(T)),if_defined(term_is_ft(I,T)))*->true;type_deduced(I,T) ).
...
isa_asserted_1(_,T):- tE(completelyAssertedCollection,T),!,fail.
...
isa_w_type_atom(I,T):- tE(functorDeclaresPred,T),!,isa(I,T).
...
decl_type_unsafe(Spec):- !, must((with_assertions([thlocal:infSkipArgIsa,thlocal:infSkipFullExpand],(add(isa(Spec,tCol)))))).
...
define_ft_0(xyzFn):-!.
define_ft_0(Spec):- tE(ttFormatType,Spec),!.
define_ft_0(Spec):- tE(tCol,Spec),dmsg(once(maybe_coierting_plain_type_to_formattype(Spec))),fail.
...
% ISA CALL
isa_provide_mpred_storage_op(call(_),G):- was_isa(G,I,C),!, (isa_backchaing(I,C);tE(C,I)).
% ISA CLAUSES
isa_provide_mpred_storage_clauses(isa(I,C),true,hasInstanceIC):-tE(C,I).
isa_provide_mpred_storage_clauses(H,true,hasInstanceCI):-
   (compound(H)-> 
      ((functor(H,C,1)-> H=..[C,I]; H=isa(I,C)), tE(C,I)) ; 
      (tE(C,I),(nonvar(C)->append_term(C,I,H);H=isa(I,C)))).
...
assert_isa_i(I,PT):- nonvar(PT),tE(functorDeclaresPred,PT),!,decl_mpred(I,PT),assert_hasInstance(PT,I).
...
assert_isa_ilc(I,T):- not(not(tE(T,I))),!.
...
assert_isa_ilc_unchecked(I,_):- not(isa(I,_)),not(tE(tCol,I)),show_call_failure(assert_if_new(user:hasInstance_dyn(tCountable,I))),fail.
...
is_Template(I):- get_mpred_arg(_,I,Arg1),tE(tCol,Arg1).
...
assert_isa_hooked(F,T):- tE(functorDeclaresPred,T),decl_mpred(F,T),fail.
...
assert_isa_hooked_after(F,T):-tE(functorDeclaresPred,T),!,decl_mpred(F,T).
...
:-pfc_add('=>'(isa(Spec,T)/(get_functor(Spec,F),F\=Spec),tFunction(F)), (formatted_resultIsa(Spec,T),resultIsa(F,T))).
...
tCol(prologHybrid).
tCol(pfcControlled).
:-pfc_add( pfcControlled(genlPreds/2)).
:-pfc_add( pfcControlled(isa/2)).
:-pfc_add(pfcControlled(argIsa/3)).

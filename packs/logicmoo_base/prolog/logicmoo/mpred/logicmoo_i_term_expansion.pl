...
db_expand_0(_ ,NC,NC):- (\+ compound(NC)),!.
...
% db_expand_0(Op,decl_mpred(F,Props),O):-!,db_expand_0(Op,user:mpred_prop(F,Props),O).
...
%typePropsToType(user:mpred_prop,tPred).
...
expand_props(_,Sent,OUT):- (\+ compound(Sent)),!,OUT=Sent.
...
expand_props(Op,props(Obj,PropVal),OUT):- safe_univ(PropVal,[Prop,NonVar|Val]),Obj==NonVar,!,into_expand_mpred_form(Op,[t,Prop,Obj|Val],OUT).
...
expand_props(Op,props(Obj,PropVal),OUT):- PropVal=..[Prop|Val],not(infix_op(Prop,_)),!,into_expand_mpred_form(Op,[t,Prop,Obj|Val],OUT).
expand_props(Op,props(Obj,PropVal),OUT):- PropVal=..[Prop|Val],!,trace_or_throw(dtrace),into_expand_mpred_form(Op,[t,Prop,Obj|Val],OUT).
...
:-export(db_quf/4).
db_quf(_ ,C,Pretest,Template):- (\+ compound(C)),!,must(Pretest=true),must(Template=C).
...
into_mpred_form(V,VO):- (\+ compound(V)),!,VO=V.
...
foreach_arg(ARGS,_N,_ArgIn,_ArgN,_ArgOut,_Call,ARGS):- (\+ compound(ARGS)),!.
...
transform_holds_3(_,A,A):- (\+ compound(A)),!.
...
mud_pred_expansion(_Prd,_HNH,G1,_):- (\+ compound(G1)),!,fail.

...
% :- context_module(M),M:with_pfc_preds_module(listing,M).
...
(neg(P), P) => conflict(P).
(P , neg(P)) => conflict(P).
% ((neg(P)/{pfc_literal_or_neg_holder(P)}, P) => conflict(P)).
%((P/{pfc_literal_or_neg_holder(P)} ,neg(P)) => conflict(P)).
...
db_uclasue(H,B):- must(pfc_local(H)),
...
mpred_uclasue(H,B):- must(pfc_local(H)),
...
unused_db_clause_ref(H,B,Ref):-must(pfc_local(H)),!,pfc_clause_localdb_ref(H,B,Ref).

unused_pfc_clause_localdb_ref(H,B,Ref):- copy_term(H:B,HH:BB),clause(HH,BB,Ref),clause(CH,CB,Ref),H:B=@=CH:CB,!.

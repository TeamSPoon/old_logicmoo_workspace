...
%:- include('../../../src_mud/server/mud_header.pl').
...
%% pfcAdd/2 and fcPost/2 are the main ways to db_assert new clauses into the
%% database and have forward reasoning done.

%% pfcAdd(P,S) asserts P into the dataBase with support from S.

pfcAdd(P) :-  pfcAdd(P,(user,user)).
...
% fcPost(+Ps,+S) tries to pfcAdd a fact or set of fact to the database.  For
...
% fcPost1(+P,+S) tries to pfcAdd a fact to the database, and, if it succeeded,
...
fcPost1(_,_).
%%fcPost1(P,S) :-  pfcWarn("pfcAdd(~w,~w) failed",[P,S]).
...
pfcEnqueue(P,S) :-
  pfc_search(Mode) 
	...
	else         -> pfcWarn("Unrecognized pfc_search mode: ~w", Mode))
     ; pfcWarn("No pfc_search mode").
...
pfc_halt(Format,Args) :- 
  format(Format,Args),
  pfc_haltSignal -> 
       pfcWarn("pfc_halt finds pfc_haltSignal already set")
...
pfcAddTrigger(trigPos(Trigger,Body),Support) :-
  !,
...
pfcAddTrigger(trigNeg(Trigger,Test,Body),Support) :-
  !,
...
pfcAddTrigger(trigBC(Trigger,Body),Support) :-
  !,
...
pfcBtPtCombine(Head,Body,Support) :- 
  ...
  %% find any trigPos's with unifying heads and pfcAdd the instantied trigBC body.
...
fcUndo(trigPos(Key,Head,Body)) :-  
  ...
  (db_retract(trigPos(Key,Head,Body))
    -> unFc(trigPos(Head,Body))
     ; pfcWarn("Trigger not found to db_retract: ~w",[trigPos(Head,Body)])).
...
fcUndo(trigNeg(Head,Condition,Body)) :-  
  ...
  (db_retract(trigNeg(Head,Condition,Body))
    -> unFc(trigNeg(Head,Condition,Body))
     ; pfcWarn("Trigger not found to db_retract: ~w",[trigNeg(Head,Condition,Body)])).
...
pfc_supported(local,P) :- !, pfcGetSupport(P,_).
pfc_supported(cycles,P) :-  !, wellFounded(P).
pfc_supported(_,_P) :- true.
...
pfcDefineBcRule(Head,Body,ParentRule) :-
  ...
  pfcForEach(pfc_nf(Body,Lhs),
          (buildTrigger(Lhs,rhs(Rhs),Trigger),
           pfcAdd(trigBC(Head,Trigger),(ParentRuleCopy,user)))).
...
pfcType(('<=>'(_,_)),Type) :- !, Type=rule.
pfcType(('<='(_,_)),Type) :- !, Type=rule.
pfcType(trigPos(_,_,_),Type) :- !, Type=trigger.
pfcType(trigPos(_,_),Type) :- !, Type=trigger.
pfcType(trigNeg(_,_,_),Type) :- !,  Type=trigger.
pfcType(trigBC(_,_),Type) :- !,  Type=trigger.
...
pfcAddSupport(P,(Fact,Trigger)) :-
  db_assert(support1(P,Fact,Trigger)),
...
pfcReset :-
  db_clause(support1(P,F,Trigger),true),
  pfcRetractOrWarn(P),
...
pfcRetractOrWarn(X) :- 
  pfcWarn("Couldn't db_retract ~p.",[X]).
...
:- pfc_default(pfcWarnings(_), pfcWarnings(true)).
...
pfc_spy(Form) :- pfc_spy(Form,[pfcAdd,pfcRem],true).

pfc_spy(Form,Modes) :- pfc_spy(Form,Modes,true).

pfc_spy(Form,[pfcAdd,pfcRem],Condition) :-
  !,
  pfc_spy1(Form,pfcAdd,Condition),
...
pfcWarn :- 
  db_retractall(pfcWarnings(_)),
  db_assert(pfcWarnings(true)).

nopfcWarn :-
  db_retractall(pfcWarnings(_)),
  db_assert(pfcWarnings(false)).

...
term_depth(C,TD):-hotrace(term_depth0(C,TD)).
...
memoize_pos_to_db(WHY,CYCPOS,W2,W1):- W2= w(W1,DATA),
...
thing_LF(capital,feature&city,X,isa_capital(X),[],_).
...
/* THAT IS HAD */
/*
trans_LF(have,feature&mpred,X,feature&type,Y,hasProperty(Y,X),[],_,_).
verb_root_db(have).
regular_pres_db(have).
regular_past_db(had,have).
...
name_db(Name) :- thlocal:useAltPOS, loop_check(not(cw_db(Name,_))).
...
name_template_db(X,feature& ISA) :- plt_call(X,'Noun',( nonvar(ISA), isa(X,ISA),not_ccw(X))).
name_template_db(X,feature& ISA & _) :- plt_call(X,'Noun',( nonvar(ISA), isa(X,ISA),not_ccw(X))).
name_template_db(X,feature& _ & ISA ) :-plt_call(X,'Noun',( nonvar(ISA), isa(X,ISA),not_ccw(X))).
...
ocw_db3(W,SPOS):-plt_call(W,POS,(( 'suffixString'(CycWord,String),String\='',atom_concat(_First,String,W),'derivationalAffixResultPOS'(CycWord,POS),simplePOS(POS,SPOS)))).
...
verb_type_db_0(Verb,main+ditrans):- plt_call(Verb,'Verb',  ditrans_LF(Verb,_,_,_,_,_,_,_,_,_,_,_)).
...
verb_form_db(Active,Verb,pres+part,_):-plt_call(ok(Active),'Verb',((nop(active),talk_db(_,Verb,_VerbPL,_Imperfect,Active,_PastPart)))).
verb_form_db(VerbPL,Verb,pres+fin,_+pl):-fail,plt_call(VerbPL,'Verb',(nop(verb_ptl),talk_db(_,Verb,VerbPL,_Imperfect,_Active,_PastPart))).
...
verb_form_db(PastPart,Verb,past+part,_):-plt_call(ok(PastPart),'Verb',(nop(past_part),talk_db(_,Verb,_VerbPL,_Imperfect,_Active,PastPart))).
...
must_test_801([does, afghanistan, border, china, ?], [sent([does, afghanistan, border, china, ?]), parse(q(s(np(3+sg, name(afghanistan), []), verb(border, active, pres+fin, [], pos), [arg(dir, np(3+sg, name(china), []))], []))), sem((answer80([]):-borders(afghanistan, china))), qplan((answer80([]):-{borders(afghanistan, china)})), 
...
must_test_801([is, there, more, than, one, country, in, each, continent, ?], [sent([is, there, more, than, one, country, in, each, continent, ?]), parse(q(s(there, verb(be, active, pres+fin, [], pos), [arg(dir, np(3+sg, np_head(quant(more, nb(1)), [], country), [pp(prep(in), np(3+sg, np_head(det(each), [], continent), []))]))], []))), sem((answer80([]):- \+A^ (continent(A), \+C^ (numberof(B, (country(B), in(B, A)), C), C>1)))), qplan((answer80([]):- \+D^ (continent(D), \+F^ (numberof(E, (country(E), in(E, D)), F), F>1)))), 
...
must_test_801([what, countries, are, there, in, europe, ?], [sent([what, countries, are, there, in, europe, ?]), parse(whq(feature&place&country-B, s(np(3+pl, np_head(int_det(feature&place&country-B), [], country), []), verb(be, active, pres+fin, [], pos), [void], [pp(prep(in), np(3+sg, name(europe), []))]))), sem((answer80([A]):-country(A), in(A, europe))), qplan((answer80([A]):-in(A, europe), {country(A)})), 

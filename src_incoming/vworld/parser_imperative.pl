...
member_ci(L,ListI):-string_ci(L,LL1),member(LL2,ListI),string_equal_ci(LL1,LL2).
...
string_equal_ci(L0,L1):-string_ci(L0,SL0),string_ci(L1,SL0).
...
parse_agent_text_command(Agent,SVERB,ARGS,NewAgent,GOAL):-
   parse_verb_phrase(Agent,do(NewAgent,GOAL),[SVERB|ARGS],[]).
...
parse_agent_text_command_1(Agent,SVERB,ARGS,Agent,GOAL):-
   parse_verb_phrase_real(Agent,SVERB,ARGS,GOALANDLEFTOVERS),
...
starts_with_icase(A,B):-string_ci(A,UA),string_ci(B,UB),(atom_concat(UA,_,UB);atom_concat(UB,_,UA)).

...
term_listing(Atom,List):-atomic(Atom), any_to_atom(Atom,C),term_listing(C,[contains(C)|List]).
...
use_term_listing_1((HO),H,B):- use_term_listing_2((HO),H,B).


use_term_listing_2((HO),H,B):- term_to_atom((H:-B),H1B1), (sub_atom_icasechk(HO,_,H1B1);sub_atom_icasechk(H1B1,_,HO)),!.

...
argIsa_call_0(Arity,N,T):-mpred_arity_pred(Arity),!,arg(N,vv(mpred,int,int),T).
argIsa_call_0(F,N,Type):-moo:type(F),!,(N=1 -> Type=F;Type=term(voprop)).
argIsa_call_0(F,_,term):-member(F/_,
                                [
                                argIsa/3,
                                assert_with_pred/2,
                                 negate_wrapper0/2,

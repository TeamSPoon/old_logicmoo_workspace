...
user:file_search_path(tptp, '.').
axiom_path(EnvPath):-user:file_search_path(tptp, EnvPath).
...
:- dynamic(dnfxref/2). %crossref authorial sources (ussually to fof/3)
...
tptp2leancop_main1(File,Settings):-
	   ...
	   save_leancop_bootstrap('../src_incoming/dbase/dbase_i_bootstrap_dnf.pl',R).	   
...
save_leancop(File,List) :-
 tell(File),
 forall(member(L,List),(copy_term(L,C), snumbervars(C),'format'('~q.~n',[C]))),
...
leancop_tptp2_(File,AxPath,AxNames,IDs,F,Con) :-
    ...
    ( fof2cop_(Stream,AxPath,AxNames,IDs,F,Con) -> close(Stream) ; close(Stream), fail ).

fof2cop_(Stream,AxPath,AxNames,IDs,F,Con) :-
    read(Stream,Term),
    ( Term=end_of_file -> IDs=[], F=[], Con=[] ;
    fof2cop_term(Stream,Term,AxPath,AxNames,IDs,F,Con)).

fof2cop_term(Stream,STerm,AxPath,AxNames,IDs,F,Con):-
   nnf_pre_clean(sent,STerm,Term,_),!,
   fof2cop_term_1(Stream,Term,AxPath,AxNames,IDs,F,Con).
...
logical_functor_here(fof).
logical_functor_here(A):-logical_functor(A).


nnf_pre_clean(_Type,Atom,Atomic,Vars):-atom(Atom),nnf_pre_clean_atom(Atom,Atomic,Vars).
nnf_pre_clean(_Type,Atomic,Atomic,[]):-not(compound(Atomic)),!.
...
nnf_pre_clean_functor(X,Y,Vars):-atom(X),nnf_pre_clean_atom(X,Y,Vars).
nnf_pre_clean_functor(X,X,[]).
nnf_pre_clean_atom(X,S,Vars):-atom_concat('s__',N,X),!,nnf_pre_clean_atom(N,S,Vars).
nnf_pre_clean_atom(X,S,Vars):-atom_concat(N,'__m',X),!,nnf_pre_clean_atom(N,S,Vars).
nnf_pre_clean_atom(X,S,Vars):-atom_concat(N,'__t',X),!,nnf_pre_clean_atom(N,S,Vars).
nnf_pre_clean_atom(X,X,[]).
...
%% dumpterms(DumpFile1)
%% dumpdnf2axioms(DumpFile2)
:- tptp2leancop(['/devel/TPTP/Problems/CSR/CSR075+2.p'],[dumpsyms(d1),dumpterms(d2),dumpdnf2axioms(d3)]).

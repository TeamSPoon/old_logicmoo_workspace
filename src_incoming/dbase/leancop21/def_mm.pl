...
%
%  Settings: list of settings, which can contain leancop21_def, nodef and conj;
%            if it contains nodef/leancop21_def, no definitional transformation
...
leancop21_make_matrix(Fml,Matrix,Set) :-
    univar(Fml,[],F1),
    ( member(conj,Set), F1=(A=>C) -> F2=((A,#)=>(#,C)) ; F2=F1 ),
    ( member(nodef,Set) ->
       def_nnf(F2,NNF,1,_,nnf), dnf(NNF,DNF)
       ;
       \+member(leancop21_def,Set), F2=(B=>D) ->
        def_nnf(~(B),NNF,1,I,nnf), dnf(NNF,DNF1),
        def_nnf(D,DNF2,I,_,leancop21_def), DNF=(DNF2;DNF1)
        ;
        def_nnf(F2,DNF,1,_,leancop21_def)
    ),
    mat(DNF,M),
...
%
%  Example: def_nnf(ex Y:(all X:((p(Y) => p(X)))),DEF,leancop21_def).
%           DEF = ~ p(X1) ; p(1 ^ [X1])

leancop21_def_nnf(Fml,DEF,I,I1,Set) :-
    leancop21_def(Fml,[],NNF,DEF1,_,I,I1,Set), leancop21_def(DEF1,NNF,DEF).

leancop21_def([],Fml,Fml).
leancop21_def([(A,(B;C))|DefL],DEF,Fml) :- !, leancop21_def([(A,B),(A,C)|DefL],DEF,Fml).
leancop21_def([A|DefL],DEF,Fml) :- leancop21_def(DefL,(A;DEF),Fml).

leancop21_def(Fml,FreeV,NNF,DEF,Paths,I,I1,Set) :-
    ( Fml = ~(~A)      -> Fml1 = A;
      ...
      Fml = ~((A => B))-> Fml1 = ((A , ~B));
      Fml = (A <=> B)  ->
      ( Set=leancop21_def        -> Fml1 = ((A => B) , (B => A));
                          Fml1 = ((A , B) ; (~A , ~B)) );
      Fml = ~((A<=>B)) -> Fml1 = ((A , ~B) ; (~A , B)) ), !,
    leancop21_def(Fml1,FreeV,NNF,DEF,Paths,I,I1,Set).

leancop21_def((ex X:F),FreeV,NNF,DEF,Paths,I,I1,Set) :- !,
    leancop21_def(F,[X|FreeV],NNF,DEF,Paths,I,I1,Set).

leancop21_def((all X:Fml),FreeV,NNF,DEF,Paths,I,I1,Set) :- !,
    copy_term((X,Fml,FreeV),((I^FreeV),Fml1,FreeV)), I2 is I+1,
    leancop21_def(Fml1,FreeV,NNF,DEF,Paths,I2,I1,Set).

leancop21_def((A ; B),FreeV,NNF,DEF,Paths,I,I1,Set) :- !,
    leancop21_def(A,FreeV,NNF1,DEF1,Paths1,I,I2,Set),
    leancop21_def(B,FreeV,NNF2,DEF2,Paths2,I2,I1,Set),
    append(DEF1,DEF2,DEF), Paths is Paths1 * Paths2,
    (Paths1 > Paths2 -> NNF = (NNF2;NNF1);
                        NNF = (NNF1;NNF2)).

leancop21_def((A , B),FreeV,NNF,DEF,Paths,I,I1,Set) :- !,
    leancop21_def(A,FreeV,NNF3,DEF3,Paths1,I,I2,Set),
    ( NNF3=(_;_), Set=leancop21_def -> append([(~I2^FreeV,NNF3)],DEF3,DEF1),
                             NNF1=I2^FreeV, I3 is I2+1 ;
                             DEF1=DEF3, NNF1=NNF3, I3 is I2 ),
    leancop21_def(B,FreeV,NNF4,DEF4,Paths2,I3,I4,Set),
    ( NNF4=(_;_), Set=leancop21_def -> append([(~I4^FreeV,NNF4)],DEF4,DEF2),
...
leancop21_def(Lit,_,Lit,[],1,I,I,_).

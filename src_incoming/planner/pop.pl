...
:- op(1200,xfx,[<-,'<<']).
% N.B. we assume that conjunctions are represented as listsz
...
test_this:- [delrob_strips_PMG].

% TRY THE FOLLOWING QUERIES with delrob_strips.pl:
test_this:- solve([carrying(rob,k1)],P,3), seq(P,S).
test_this:- solve([sitting_at(k1,lab2)],P,7), seq(P,S).
test_this:- solve([carrying(rob,parcel),sitting_at(rob,lab2)],P,9), seq(P,S).
test_this:- solve([sitting_at(rob,lab2),carrying(rob,parcel)],P,9), seq(P,S).

% BLOCKS WORLD FOR PARTIAL ORDER PLANNER
test_this:- solve([on(a,f)],P,2), seq(P,S).
test_this:- solve([on(b,f)],P,4), seq(P,S).
test_this:- solve([on(c,d)],P,8), seq(P,S).
test_this:- solve([on(b,f),on(f,a)],P,6), seq(P,S). % "sussman's anomoly"
test_this:- solve([on(a,d),on(f,b)],P,6), seq(P,S). % "block swapping"
test_this:- solve([on(b,table),on(f,e)],P,6), seq(P,S). % independent problems

% For soccer
test_this:- solve([waiting_at(ball,oppGoal)],P,7), seq(P,S).
test_this:- solve([waiting_at(kula,cell(3,1))],P,2), seq(P,S).

% La siguiente consulta no esta andando:
test_this:- solve([waiting_at(kula,cell(2,3))],P,2), seq(P,S).


:-forall(test_this,true).

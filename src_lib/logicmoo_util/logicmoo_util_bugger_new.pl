...
mcall(Goal):- in_mcall((menv(Env),mcall(Goal, Env))).
...
mcall(!, _Env):-!,trace, ( true ; throw(cut(_)) ).
...
:-export(mcall2/2).
mcall2(Goal,Env):- trace,
    functor(Goal,F, _),
    nod('Call: ', Goal, Env),    
    catch( ( redo_clause(Env, Goal, F, BodyOut),call(trace,BodyOut) ), cut(F), fail),
    nod('Exit: ', Goal, Env).
mcall2(Goal, Env):-
    nod('Fail: ', Goal, Env),
    fail.
...
nod(Message, Goal, Env):- arg(1,Env,List),length(List,Tab),tab(Tab), write(Env), write(': '), write(Message),
    write(Goal), nl.
...
mcall_expansion(G,_M,_Call,_,G):-!.
...
:-visible(+all).
:-leash(+all).
user:goal_expansion(Goal,G0):- \+inxp, wyes(inxp,mcall_expansion(Goal,G0)),!.
system:goal_expansion(Goal,G0):- \+inxp, wyes(inxp,mcall_expansion(Goal,G0)),!.
user:term_expansion(Goal,G0):- \+inxp, wyes(inxp,mcall_expansion(Goal,G0)),!.
system:term_expansion(Goal,G0):- \+inxp, wyes(inxp,mcall_expansion(Goal,G0)),!.

...
draw_line:- (t_l:print_mode(H)->true;H=unknown),fmt("~N%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%~n",[]).
...
mpred_listing(What):-t_l:mpred_listing_disabled,!.
mpred_listing(What):-loop_check(mpred_listing_nlc(What)).

:-meta_predicate(mpred_listing_nlc(?)).

mpred_listing_nlc(M:What):-atom(M),!,M:mpred_listing(What).
mpred_listing_nlc(What):-loop_check(mpred_listing_0(What),true).

mpred_listing_0(What):-get_pi(What,PI),PI\=@=What,mpred_listing(PI).
mpred_listing_0(What):-nonvar(What),What=neg(Then),!, \+ \+ mpred_listing_1(Then), \+ \+ mpred_listing_1(What).
mpred_listing_0(What):- \+ \+  mpred_listing_1(neg(What)), \+ \+ mpred_listing_1(What).

mpred_listing_types('Triggers').
mpred_listing_types('Instances').
mpred_listing_types('Subclasses').
mpred_listing_types('ArgTypes').
mpred_listing_types('Arity').
mpred_listing_types('Forward').
mpred_listing_types('Bidirectional').
mpred_listing_types('Backchaining').
mpred_listing_types('Negative').
mpred_listing_types('Sources').
mpred_listing_types('Supports').
mpred_listing_types('Edits').
...
mpred_listing_1(neg(What)):-var(What),!.
mpred_listing_1(neg(_What)):-!.
mpred_listing_1(What):-var(What),!.
mpred_listing_1(What):-
...
lmhook:shared_hide_data('$was_imported_kb_content$'/2):- !,listing_filter(hideMeta).
lmhook:shared_hide_data(spftY/4):- !,listing_filter(hideTriggers).
lmhook:shared_hide_data(spft/3):- !,listing_filter(hideTriggers).
lmhook:shared_hide_data(nt/3):- !,listing_filter(hideTriggers).
lmhook:shared_hide_data(pt/2):- !, listing_filter(hideTriggers).
lmhook:shared_hide_data(bt/2):- !, listing_filter(hideTriggers).
lmhook:shared_hide_data((H:-
 cwc,
...
lmhook:shared_hide_data(pfcMark/4):- !,listing_filter(hideMeta).

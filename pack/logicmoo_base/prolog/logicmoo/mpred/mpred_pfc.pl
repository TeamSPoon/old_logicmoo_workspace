...
mpred_assert_w_support(P,Support):- 
  (mpred_clause_u(P) ; assert_u_confirmed(P)),
  !,
...
mpred_add_support(P,(Fact,Trigger)):-
 ...
 %     [F,Condition,Action,mpred_add_support(P,(Fact,Trigger))]));true),
  assert_u_confirmed(spft(P,Fact,Trigger)).

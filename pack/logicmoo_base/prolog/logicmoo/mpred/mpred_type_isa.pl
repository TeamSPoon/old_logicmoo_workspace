...
% padd(Obj,height(ObjHt))  == padd(height,Obj,ObjHt,...) == ain_post(QueryForm)
...
% Module Local Init.
%
baseKB:module_local_init:- ain_post((isa(I,T):- cwc,isa_backchaing(I,T))).
...
isa_asserted_0(I,C):- nonvar(C),clause_b(completelyAssertedCollection(C)),!,no_loop_check(isa_asserted_1(I,C)).
isa_asserted_0(I,C):- nonvar(I),clause_b(completeIsaAsserted(I)),!,tCol_gen(C),get_mudIsa(I,C).
...
isa_backchaing_n_v(I,C):-  
   (get_mudIsa(I,T) *->  (C=T ;transitive_superclass_not_same(T,C)) ;
...
isa_asserted_1(I,C):- get_mudIsa(I,C).
isa_asserted_1(I,C):- (nonvar(C),append_term(C,I,HEAD),current_predicate(_,baseKB:HEAD))->loop_check(call_u(HEAD)).

get_mudIsa(I,C):- clause_b(mudIsa(I,C)).
/*
...
isa_asserted_1(I,C):- !, get_mudIsa(I,C)*->true;isa_asserted_2(I,C).
isa_asserted_1(I,C):- 
...
guess_types(W,What):- ain_post(tried_guess_types_from_name(W)),ignore((atom(W),guess_types_0(W,What))).
...
guess_types_0(W,ftID):-hotrace((atom(W),atom_concat(i,T,W), 
   ...
   decl_type_safe(NewType),(a(tCol,NewType)->(assert_isa_safe(W,NewType),ain_post(did_learn_from_name(W)),guess_supertypes(NewType));true).
...
guess_supertypes(W):- ain_post(tried_guess_types_from_name(W)),ignore((atom(W),guess_supertypes_0(W))).
...
%
guess_supertypes_0(W):-atom(W),atomic_list_concat(List,'_',W),length(List,S),S>2,!, append(FirstPart,[Last],List),atom_length(Last,AL),AL>3, 
  \+ (member(flagged,FirstPart)),
            atomic_list_concat(FirstPart,'_',_NewCol),fail,ain_guess(genls(W,Last)),ain_post(did_learn_from_name(W)).
guess_supertypes_0(W):-T=t,to_first_break(W,lower,T,All,upper),to_first_break(All,upper,_UnusedSuper,Rest,_),
  ...
  ain_guess(isa(W,SuperTT)),ain_post(did_learn_from_name(W)),!,guess_typetypes(SuperTT).
...
% Assert If New Guess.
%
ain_guess(G):-show_call(ain_guess,mpred_post(G)).
ain_post(G):- show_call(ain_post,mpred_post(G)).
...
guess_typetypes(W):- ain_post(tried_guess_types_from_name(W)),ignore((atom(W),guess_typetypes_0(W))).
...
guess_typetypes_0(TtTypeType):-
 ...
 atom_concat(t,Type,TType),ain_post((isa(T,TtTypeType)==>genls(T,TType))).

/*
system:term_expansion(isa(Compound,PredArgTypes),
  (:-dmsg(ain_post(wizza(Compound,PredArgTypes))))):-
...
decl_type(Spec):- show_call(why,ain_post(tCol(Spec))),guess_supertypes(Spec).
...
assert_isa_rev(T,I):- current_prolog_flag(unsafe_speedups,true),ain_post(isa(I,T)),sanity(isa_asserted(I,T)),!.
...
% :- ain_post((baseKB:isa(I,C):-loop_check(isa_backchaing(I,C)))).
% baseKB:module_local_init:- ain_post(('$toplevel':isa(I,C):-baseKB:isa(I,C))).
...
baseKB:module_local_init(_UserModule,SystemModule):-ain_post(SystemModule:tCol(tCol)).
baseKB:module_local_init(_UserModule,SystemModule):-ain_post(SystemModule:tCol(ttPredType)).
...
isa_asserted(I,C):-  get_mudIsa(I,C).

...
pttp_assert_wid(ID,pttp,X):- must((bugger:with_assertions(thlocal:current_local_why(ID,X), must(( pttp1_wid(ID,X,Y), pttp2_wid(ID,Y)) )))).
...
must_pttp_id(ID):-must(thlocal:current_local_why(ID,_)).

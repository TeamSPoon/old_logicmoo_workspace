...
env_asserta(F):-asserta(F).
env_assert(F):-assert(F).
env_retract(F):-retract(F).
env_retractall(F):-retractall(F).
...
:-env_retractall(solution_file(_)).
:-env_asserta(solution_file(user)).

%:-set_prolog_flag(unknown,fail).
solve:- with_filematch([wfm(test_hyhtn)]), doall(solve(_)).
solve(Id) :-
	no_repeats(htn_task(Id,Goal,Init);planner_task(Id,Goal,Init)),
...
clean:-
	...
	env_assert(op_num(0)),
	env_assert(my_stats(0)).

show_time(Call):- functor(Call,F,A), time(Call),format('~N% ~q~n',[previous_statistics_for(F/A)]).
planner_interface_0(G,I, SOLN,NODES):-
        env_retract(op_num(_)),env_assert(op_num(0)),
        once(ignore(env_retract(my_stats(_)))),
        gripe_time(1,show_time(ground_hierarchy)),
        gripe_time(1,show_time(change_op_representation)),
        must(find_relate_state),
        statistics(runtime,[_,Time]),
        env_assert(my_stats(Time)),!,
        must_det(make_problem_into_node(I, G, Node)),
        env_assert(Node),!,
        dmsg(solving_for(G)),
	start_solve(SOLN,NODES).
planner_interface(G,I, SOLN,NODES):-
   (planner_interface_0(G,I, SOLN,NODES)*->true;(tell(user),nl,write('failure in initial node'))).
...
start_solve(Sol,OPNUM):-   
   ...
   write('Solution:\n '),writeq(Sol),nl,
   write('***************************************'),
   env_assert(time_taken(CP)),  
   env_assert(soln_size(SIZE)),
   env_assert(plan_used(OPNUM)),
   env_retractall(tn(_,_,_,_,_,_)),!.
...
%   nl,write('step '),write(HPid),
   env_retract(op_num(N)),
   N1 is N+1,
   env_assert(op_num(N1)),
...
expand_hp(HPid,TN,ACH,Pre,Post,State,Statics,Statics1):-
   ...
   env_assert(tp_node(TP,Pre,Post,Statics,Score,Steps)),
...
get_all_related_goal(Goal):-
    env_retractall(goal_related(_)),
    env_assert(goal_related(Goal)),
    get_all_related_goal1(Goal,States),
    env_retractall(goal_related(_)),
    env_assert(goal_related(States)),!.
...
get_all_related_goal1([se(Sort,Obj,SE)|Rest],States):-
    ...
    env_retract(goal_related(List)),
    set_append_e(List,OPre,List1),
    env_assert(goal_related(List1)),
    fail.
...
fwsearch(TN,ST,State):-
   env_retract(solved_node(ST,step(HP,Name,Pre,State,exp(TN)))).
fwsearch(TN,ST,State):-
   select_tnode(tp_node(TP,Pre,Post,Statics,Score,Dec)),
   env_assert(closed_node(TP,Pre,Post,Statics,Score,Dec)),
...
select_tnode(tp_node(HPid,Pre,Post,Statics,Score,Dec)) :-
   env_retractall(score_list(LS)),
   env_assert(score_list([])),
   lowest_score(Score),
   env_retract(tp_node(HPid,Pre,Post,Statics,Score,Dec)),!.
%   tell(user),nl,write('new level'),nl,told,!.

% find the lowest_score of tp_node
lowest_score(LScore):-
     tp_node(HPid,Pre,Post,Statics,Score,Dec),
     env_retract(score_list(LS)),
     env_assert(score_list([Score|LS])),
     fail.
lowest_score(LScore):-
     env_retract(score_list(D)),
     sort(D,[LScore|SD]).

:- style_check(-discontiguous).
% env_assert tp_node if all HP expand, env_assert solved_node
assert_tnode(TP,Pre,Post,Statics,Score1,Dec):-
   all_HP_expanded(Dec),
   combine_exp_steps2(Dec,OneDec),
   env_assert(solved_node(Statics,OneDec)),!.
...
assert_tnode1(TP,Pre,Post,Statics,Score,ExpDec,UnexpDec):-
   ...
   env_assert(tp_node(TP1,Pre,Post,Statics,Score1,Dec)),!.
...
combine_steps1(step(HP1,Name1,Pre1,Post1,exp(TN1)),step(HP2,Name2,Pre2,Post2,exp(TN2)),step(HP,achieve(Post2),Pre1,Post2,exp(TN))):-
    ...
    env_assert(tn(TN,achieve(Post0),Pre0,Post0,[before(TN1,TN2)],[step(HP1,Name1,Pre1,Post1,exp(TN1)),step(HP2,Name2,Pre2,Post2,exp(TN2))])),!.
...
delete_all_nodes :-
	...
	env_retractall(tp_node(_,_,_,_,_,_)),
	env_retractall(closed_node(_,_,_,_,_,_)),
	env_retractall(solved_node(_,_,_)).
delete_all_nodes :- !.
...
% env_assert in prolog database as gpredicates(Pred,GPredls)
ground_predicates:-
     predicates(Preds),
...
collect_grounded_pred:-
     ...
     env_retractall(gpred(Pred,_)),
     env_assert(gpredicates(Pred,GPredls)),
     fail.
collect_grounded_pred.

% grounding substate_class to primary sort level
% env_assert in prolog database as gsubstate_class(Sort,Obj,States)
ground_substate_class:-
...
% env_assert grounded predicates with related upper level predicates
...
list(A) :-
        functor(A,'.',_).
% ***********************for multy tasks*****************
:- env_assert(time_taken(0)).
:- env_assert(soln_size(0)).
:- env_assert(plan_used(0)).
...
solve(FN,FN):-
   ...
   env_assert(sum(0)),
   sum_plan(PLAN),
   env_retractall(time_taken(_)),
   env_retractall(soln_size(_)),
   env_retractall(plan_used(_)),

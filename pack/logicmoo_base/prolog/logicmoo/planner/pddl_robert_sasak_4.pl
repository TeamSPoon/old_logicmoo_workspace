...
solve_files_0(DomainFile, ProblemFile):-
  format('~q.~n',[solve_files(DomainFile, ProblemFile)]),
   notrace((must_det_l(( parseDomain(DomainFile, DD),
    parseProblem(ProblemFile, PP),        
    notrace(term_to_ord_term(DD, D)),
...
try_solve(D,P,S):- portray_clause(hard_working_0:-try_solve(D,P,S)),fail.
% try_solve(D,P,S):- once(time_out(solve(D, P, S), 1000, Result)), Result == time_out, portray_clause(hard_working_1:-try_solve(D,P,S)),fail.
try_solve(D,P,S):- gripe_time(8,time_out((solve(D, P, S)), 900000, Result)),!, % time limit for a planner (was 500000)
   ((\+ is_list(S)
     -> portray_clause('failed'(Result):-try_solve(D,P,S)) ;
       ((Result=time_out)->portray_clause('failed'(Result):-try_solve(D,P,S));true))),!.

try_solve(D,P,S):- must(solve(D, P, S)).
...
solve(D, P, Solution):-
  trace,
    prop_get(init,P, I),    prop_put_bb(initState, I),
    prop_get(goal,P, G),    prop_put_bb(goalState, G),
    prop_get(metric,P, M),    prop_put_bb(metric, M),
    prop_get(actions,D, A),	prop_put_bb(actions, A),
    prop_get(objects,P, O),	prop_put_bb(objects, O),
    make_init_state(IS),
...
t2ot([H|T], R):-
    ...
    (([H|T]\=@= R) *->
    (!,nop(portray_clause(t2ot:-term_to_ord_term0([H|T], R)))); true).
...
arg_safe(I,C,E):-debug,C=[],trace_or_throw(debugOnError(arg(I,C,E))).
arg_safe(I,C,E):-debug,debugOnError(arg(I,C,E)).



prop_put_bb(Name, V):- bb_put(Name, V).


term_to_ord_term(I,I):-var(I),!.
term_to_ord_term(I,O):-copy_term_spec_q(I,M), t2ot(M,O),!.
...
:-user:pfc_add(prologHybrid(user:named_arguments_template(ftCompound, ftAtom, ftInt, ftCompound))).

user:named_arguments(domain(domain_name, requires, types, constants, predicates, functions, constraints, actions)).
user:named_arguments(problem(problem_name, uses_domain, requires, objects, init, goal, constraints, metric, length)).
user:named_arguments(action(action_name,parameters,preconditions,positiv_effect,negativ_effect,assign_effect)).
...
:-forall((user:named_arguments(Names),cnames_to_sargs(Names,F,A,Struct)),asserta(user:named_arguments_template(Names,F,A,Struct))).


prop_get_bb(Name,Value):- bb_get(Name,Value).


prop_get(_,       RBTree, _  ):- (\+ \+ RBTree=[] ),!, fail.
prop_get(Name, bb,      Value):- !, must(bb_get(Name,Value)).
prop_get(_   , Atomic,  _    ):- atomic(Atomic),!,fail.
prop_get(Name, Dict,    Value):- is_dict(Dict),!,get_dict(Name,Dict,Value).
prop_get(Name, RBTree,  Value):- is_rbtree(RBTree),!,nb_rb_get_node(RBTree,Name,Value).
prop_get(Name, Struct,  Value):- user:named_arguments_template(Names,_,_,Struct), arg(N,Names,Name),!,arg(N,Struct,Value).
prop_get(Indx, Struct,  Value):- integer(Indx),!, arg(Indx,Struct,Value).
prop_get(Name, Struct,  Value):- functor(Struct,_,A),arg(A,Struct,Ref),nonvar(Ref),!,prop_get(Name,Ref,Value).
...
% domain_get_action_template(-ActionDef)
%
domain_get_action_template(A):-
    prop_get_bb(actions, As),
    member(A, As).

% current_domain_get_action_def(-ActionDef, -ActionInst).
domain_get_action_template_definition(A, ActionInst):-
    domain_get_action_template(Afree),
...
reset_statistic:-
    prop_put_bb(stat_nodes, 0),
    statistics(runtime, [T,_]),
    prop_put_bb(startTime, T).

show_statistic:-
    prop_get_bb(stat_nodes, N),
    prop_get_bb(startTime, T0),
...
forbiden_pair(R):-
    domain_get_action_template(A),
    prop_get(positiv_effect,A,PE),
    prop_get(negativ_effect,A,NE),
    member(P, PE),
    member(Q, NE),
...
forbiden_pair(MA, NR):-
    member(P-Q, MA),
    domain_get_action_template(A),
    prop_get(precondition,A,Precond),
    prop_get(positiv_effect,A,PE),
    member(R, Precond),
    member(P, PE),
...
%%
%% step(+State, -ActionInst, -NewState)
...
make_init_state(I):-
    prop_get_bb(initState,I),
    prop_get_bb(goalState,G),
    prop_put_bb(fictiveGoal, G).
...
step(State, ActionInst, NewState):-
    domain_get_action_template_definition(A, ActionInst),
    prop_get(precondition,A,P),    mysubset(P, State),  % choose suitable action
    prop_get(negativ_effect,A,NE), ord_subtract(State, NE, State2),
    prop_get(positiv_effect,A,PE), ord_union(State2, PE, NewState).
...
% parseDomain(+File, -Struct, -RestOfFile)
%
%   The same as above and also return rest of file. Can be useful when domain and problem are in one file.
%
parseDomain(File, Struct, R) :-  
    read_file(File, List),!,
    (nonvar(Struct)->true;new_struct(domain,Struct)),
    % prop_merge_svo(Struct,domain_filename,File),
...
sterm2pterm(VAR,'?'(UP)):-svar(VAR,SVAR),!,lowercase_atom(SVAR,UP).
...
sterm(_,_) --> [')'],{!,fail}.
sterm(_,[]) --> ['(',')'],!.
sterm(Struct,A) --> action_def(A),!,{prop_merge_svo(Struct,actions,A)}.
sterm(Struct,requires(R)) --> requires_def(R),!,{prop_merge_svo(Struct,requires,R)}.
sterm(Struct,types(L))                    --> ['('], skey(types,_), typed_list(name, L), [')'],!,{prop_merge_svo(Struct,types,L)}.
sterm(Struct,constants(L))                --> ['(',':',constants],  typed_list(name, L), [')'],!,{prop_merge_svo(Struct,constants,L)}.
sterm(Struct,predicates(P)) --> predicates_def(P),!,{prop_merge_svo(Struct,predicates,P)}.
sterm(Struct,[H,T]) --> skey(S,H),['('],!,zeroOrMore(sterm(Struct), T), [')'],!,{prop_merge_svo(Struct,S,T)}.
sterm(Struct,[H|T]) --> ['('],satom(H),!, zeroOrMore(sterm(Struct), T), [')'],!.
sterm(Struct,[H|T]) --> ['('],sterm(Struct,H),!, zeroOrMore(sterm(Struct), T), [')'],!.
sterm(_,N)--> satom(N).
...
skey(_,_) --> [')'],{!,fail}.
skey(S,N)                  --> [':'],!,name(S),{atom_concat(':',S,N)},!.
...
optional_toplevel(Struct,Requires,R,G) --> dcgMust(G ; []),{ prop_merge_svo(Struct,Requires,R),!}.


% domain_dcg(domain(N, R, T, C, P, F, C, S))
%
...
%
domain_dcg(Struct) --> {must(Struct = domain(N, R, T, C, P, F, C, S))},
             ['(','define', '(','domain'], name(N), [')'],{prop_merge_svo(Struct,domain_name,N)},
                  optional_toplevel(Struct,requires,R,requires_def(R)),
                  optional_toplevel(Struct,types,T,types_def(T)),           %:typing
                  optional_toplevel(Struct,constants,C,constants_def(C)),
                  optional_toplevel(Struct,predicates,P,predicates_def(P)),
                  optional_toplevel(Struct,functions,F,functions_def(F)),   %:fluents
                  %optional_toplevel(Struct,constraints,C,dconstraints_def(C)),  %:constraints
                  optional_toplevel(Struct,actions,S,zeroOrMore(structure_def,S)), [')'].


requires_def(R)          --> ['(',':','requirements'], oneOrMore(require_key, R), [')'].
...
% Actions definitons
action_def(Struct) -->   {must(Struct = action(S, L, Precon, Pos, Neg, Assign))},
...
% parseProblem(+File, -Struct, -RestOfFile).
%
%   The same as above and also return rest of file. Can be useful when domain and problem are in one file.
%
parseProblem(File, Struct, R) :-
    read_file(File, L),!,
   (nonvar(Struct)->true;new_struct(problem,Struct)),
    % prop_merge_svo(Struct,problem_filename,File),
    problem(Struct, L, R),!.
...
problem_dcg(Struct)   --> { must(Struct = problem(Name, Domain, R, OD, I, G, C, MS, LS)) } ,
                                      ['(',define,'(',problem,Name,')',                                        
                                     '(',':',domain, Domain,')'], 
                { prop_merge_svo(Struct,problem_name,Name),prop_merge_svo(Struct,uses_domain,Domain)} ,!,
            optional_toplevel(Struct,requires,R,requires_def(R)),!,
            optional_toplevel(Struct,objects,OD,objects_def(OD)),!,
            optional_toplevel(Struct,init,I,init_def(I)),!,
            optional_toplevel(Struct,goal,G,goal_def(G)),!,
            optional_toplevel(Struct,constraints,C,pconstraints_def(C) ),    %:constraints
            optional_toplevel(Struct,metric, MS, metric_spec(MS)),
            optional_toplevel(Struct,length, LS, length_spec(LS)),[')'].
...
% read_file(+File, -List).
%
read_file( File, Words) :-  exists_file(File), !, seeing(Old),call_cleanup(( see(File), get0(C), notrace(read_rest(C, Words))),( seen, see(Old))),!.
...
%parse_file(+File).
test_parse_file(F,O):- must(read_file(F, L)),!,((domain(O, L, _R1); must(((problem(O, L, _R2)))))),!.

test_parse_file(F):- test_parse_file(F,L),arg_safe(2,L,List),!,
...
:-pfc_add(( (user:named_arguments(Names)/cnames_to_sargs(Names,F,A,Struct)) => user:named_arguments_template(Names,F,A,Struct))).
...
:- repeat,debug,must(test_blocks),fail.

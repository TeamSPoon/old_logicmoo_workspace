...
objects(road_route, [road_route_21,road_route_32, 
                     road_route_31 ]).
objects(rail_route,[rail_route_2,rail_route_3 ]).
...
inconsistent_constraint([attached(A,A)]).
...
atomic_invariants([
      ...
      route_available(rail_route_3),
      connects(rail_route_3,city3_ts1,city1_ts1),
      connects(rail_route_3,city1_ts1,city3_ts1),
...
operator( move_local(V, O, L), 
         [],
         [sc(truck,V,[at(V,O),
             moveable(V),
             in_city(O,City),
             in_city(L,City),ne(O,L)]
            =>[at(V,L)]  )],
         [ sc(package,X,[loaded(X,V),certified(X),at(X,O)] 
            =>[loaded(X,V),certified(X),at(X,L)])   ]
).

 ...
 * Variables i.e. first Character UpperCase letter 
 * then any sequence of characters digits underscores
 * @author Ron Simpson
 * @version 0
 */

public class OCLVariableDocument extends PlainDocument{
	...
	if (length == 0)
	    return;
	if (! Character.isUpperCase(proposedValue.charAt(0))) {
	    throw new OCLFormatException("Illegal first variable character");
	} else {
		...
		if (Character.isLetterOrDigit(proposedValue.charAt(i)) ||
		    ...
		    throw new OCLFormatException("Illegal variable character");
	}
    }
}

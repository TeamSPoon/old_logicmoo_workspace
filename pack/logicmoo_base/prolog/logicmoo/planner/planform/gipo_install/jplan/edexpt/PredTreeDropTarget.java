...
public class PredTreeDropTarget implements DropTargetListener, 
    PropertyChangeListener {
    public PredTreeDropTarget(PredTree tree) {
	this.tree = tree;	
	...
	if (propertyName.equals("enabled")) {
	    // Enable the drop target if the PredTree is enabled
	    // and vice versa.		
	...
	if (!acceptableType || 
	    ...
	    // Can't drag to a read-only PredTree
	    dtde.rejectDrag();
	...
	if (dtde.isDataFlavorSupported(DataFlavor.stringFlavor)) {
	    acceptableType = true;
	    isString = true;
	    Utility.debugPrintln("String Flavor found");
	...
	if(isString) {
	    ...
	    Utility.debugPrintln("opmaker","Got object " + data);
	    ...
	    data = (DefaultMutableTreeNode)transferable.getTransferData(SortTransferable.sortFlavor);
    ...
    /**
     * anywhere is acceptable becauese we allways add to the root
     */	
	...
	if (targetNode != null) {
		tree.addToRootAfter(src,targetNode);
	}
    }

    // Copy or move sort node 
    protected void transferSort(int action,  DefaultMutableTreeNode src,
				DefaultMutableTreeNode targetNode) {
	Utility.debugPrintln("Try to add sort to tree ");
	
	// Update the tree display
	if (targetNode != null) {
		tree.addToRootAfter(src.toString(),targetNode);
	}
    }



    protected boolean isString;         // Is node to be dropped a String
    protected PredTree tree;

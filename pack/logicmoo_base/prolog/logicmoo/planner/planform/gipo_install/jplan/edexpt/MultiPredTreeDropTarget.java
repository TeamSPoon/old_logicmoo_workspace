...
/**
 * MultiPredTreeDropTarget
 * controls dropping objects on to the multi pred tree
 * Ensures that all unifying variables take the same value
 * @author Ron Simpson
 * @version 0
 */
public class MultiPredTreeDropTarget implements DropTargetListener, 
    PropertyChangeListener {
    public MultiPredTreeDropTarget(MultiPredTree tree) {
	this.tree = tree;	
	...
	if (propertyName.equals("enabled")) {
	    // Enable the drop target if the MultiPredTree is enabled
	    // and vice versa.		
	...
	TreePath sel;
	sel = tree.getPathForLocation(loc.x,loc.y);
	Object obj = sel.getLastPathComponent();
	// Need to find if this is a leaf
	return true;
    }

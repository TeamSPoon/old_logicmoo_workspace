...
create_settings_panel(Path, Ctxt, _, _) :-
        ...
        optionmenu(Spec,'Specification',SpecVar,
                   ['Calls', 'Execution Time','Choice Points',
                    'Shallow Failures','Deep Failures','Backtracking', 'Calls+Backtracking'],
                   [calls,execution_time,choice_points,
                    shallow_fails,deep_fails,backtracks, callsbacktracks],
                   Ctxt),
...
on_event(calc(Spec,Res,SortOrder,Scale), Ctxt, _, BarVal) :- !,
        ctxt(Ctxt, [system-System,
                    header-HeaderW,
                    interp-Interp]),
        ( Spec == callsbacktracks ->            % [PM] 3.11.1 calls+backtracks
           profile_data(System, calls, Res, UnsortedBarsCalls),
           profile_data(System, backtracks, Res, UnsortedBarsBacktracks),
           sort(UnsortedBarsCalls, SortedBarsCalls),
           sort(UnsortedBarsBacktracks, SortedBarsBacktracks),
           keyed_add(SortedBarsCalls, SortedBarsBacktracks, UnsortedBars) % add counts for like keys
        ; otherwise ->                          % one of the standard specs

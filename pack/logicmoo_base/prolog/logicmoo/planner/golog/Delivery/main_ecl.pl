...
% TESTED: ECLiPSe 4.2.2 on RedHat Linux 6.2
...
% ECLiPSe Prolog lacks get0/1 which we use for debug procedures in delivery.pl
...
:- set_flag(all_dynamic,on). % ECLiPSe can't deal with predicates not being
                             % listed contiguously
:- ['../Main/lego_ecl'].     % System dependent Prolog/RCX communication
                             %     predicates
:- ['../Main/legorcx'].      % Generic Prolog/RCX communication predicates
:- ['../Main/golog'].        % IndiGolog interpreter
...
% checkKeyboardExog/1
%    Looks for exogenous events entered via keyboard.
%    Uses system-dependent I/O routines
checkKeyboardExog([KbExog]) :-
    keyboardWait(Time),
    keyboardStream(KbStream),
    select([KbStream], Time, ReadyList),
    ReadyList \==[],
    read(KbStream, Term),

...
planner_test:- solve([carrying(rob,k1)],P,3), seq(P,S).
planner_test:- solve([sitting_at(k1,lab2)],P,7), seq(P,S).
planner_test:- solve([carrying(rob,parcel),sitting_at(rob,lab2)],P,9), seq(P,S).
planner_test:- solve([sitting_at(rob,lab2),carrying(rob,parcel)],P,9), seq(P,S).

:-forall(planner_test,true).

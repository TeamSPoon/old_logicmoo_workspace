...
wpp:- defaults(Defaults),
 wpp(Defaults),
 ignore((source_file(Pred,'/opt/PrologMUD/pack/logicmoo_base/prolog/logicmoo/util/logicmoo_util_filestreams.pl'),
 forall(clause(Pred,Body),
 ((wpp((Pred:-Body),Defaults)))),fail)).


meet_term_opening_limits(Term,Options):-
   term_openings(Term,Z),
   option(term_openings(Max), Options, 99),
   
   Z<Max.

term_openings(Term,0):- \+ compound(Term),!.
term_openings(Term,Z):- functor(Term,_,A),
  
    arg(1,Term,Arg1),term_openings(Arg1,Was1), X is Was1 + 1,
    arg(A,Term,ArgA),term_openings(ArgA,WasA), Y is WasA + 1,
    max(X,Y,Z).
   
checkIndent( IndentStyle , Auto , Term):- IndentStyle==Auto ; IndentStyle=Term.

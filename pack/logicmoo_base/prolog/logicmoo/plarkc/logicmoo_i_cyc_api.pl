...
getMtForPred(CycL,Mt):-nonvar(CycL),functor(CycL,Pred,_),isRegisterCycPred_lc(Mt,Pred,_),!.
...
read_codes_one_at_a_time(Stream,[]):-at_end_of_stream(Stream),!.
read_codes_one_at_a_time(Stream,NewBuffer):-
      set_stream_buffer(Stream,[]),      
      repeat,
      read_line_to_codes_one_at_a_time_util(Stream,NewBuffer,IsDone),
      ((IsDone = true)->!;true).



read_line_to_codes_one_at_a_time_util(Stream,_,_):-
      find_stream_buffer(Stream,NewBuffer),
      NewBuffer = [Code|Buffer],char_type(Code,space),
      set_stream_buffer(Stream,Buffer),fail.

read_line_to_codes_one_at_a_time_util(Stream,NewBuffer,true):-
...
read_line_to_codes_one_at_a_time_util(Stream,NewBuffer,false):-
      find_stream_buffer(Stream,Buffer),
...
%  Read until
% ===================================================================
get_code_no_eof(InStream,NoEOFCode):- get_code(InStream,NoEOFCode), (NoEOFCode == -1 -> dmsg(throw(cyc_error('eof on',InStream))) ; true).
...
registerCycPred(Mt,Pred,Arity):-isRegisterCycPred_lc(Mt,Pred,Arity),!.
registerCycPred(Mt,Pred,Arity):-
      functor(Term,Pred,Arity),
...
assertThrough(ToMt,CycL):-
      ...
      (isRegisterCycPred_lc(Mt,Pred,Arity);atom_concat('#$',_,Pred)),!,
...
retractAllThrough(ToMt,CycL):-
      functor(CycL,Pred,Arity),
      isRegisterCycPred_lc(Mt,Pred,Arity),!,
      ignore(ToMt=Mt),
...
really_at_end_of_stream(Stream):-catch(stream_property(file_name,_Atom),E,(nop(debugFmt(E)),!,fail)).

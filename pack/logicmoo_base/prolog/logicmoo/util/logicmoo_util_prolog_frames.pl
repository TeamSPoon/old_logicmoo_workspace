...
% Stack Check.
%
stack_check:- sanity(stack_check(1600)).
...
% In Pengines.
%
in_pengines:- cnotrace(relative_frame(context_module,pengines,_)).
...
% Nth Parent Goal.
%
nth_parent_goal(Frame,Goal,Nth):- Nth>0, Nth2 is Nth-1, prolog_frame_attribute(Frame,parent,PFrame),!,cnotrace((nth_parent_goal(PFrame,Goal,Nth2))).
nth_parent_goal(Frame,Goal,_):- cnotrace((prolog_frame_attribute(Frame,goal,Goal))),!.
...
% Current Next Frames.
%
current_next_frames(Attrib,Nth,Frame,[Nth-Frame-Term|NextList]):- cnotrace((prolog_frame_match(Frame,Attrib,Term))), !,

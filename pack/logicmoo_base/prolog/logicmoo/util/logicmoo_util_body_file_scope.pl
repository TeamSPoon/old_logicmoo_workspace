...
loading_source_file(File):- loading_source_file0(File)*-> true; loading_source_file1(File).
...
%
signal_eof:- prolog_load_context(module,M), signal_eof_m(M).
...
%
signal_eof(File):- prolog_load_context(module,M), signal_eof_m(M,File).
...
%
signal_eof_m(M):- prolog_load_context(file,File), signal_eof_m(M,File).
signal_eof_m(M):- loading_source_file(File),signal_eof_m(M,File).
...
%
signal_eof_m(M,File):- module_property(M,file(File)),prolog_load_context(file,File),
     \+ \+ ((module_property(M,file(LittleFile)),LittleFile\==File)),!,
   forall(module_property(M,file(LittleFile)),signal_eof_m_1(M,LittleFile)).
signal_eof_m(M,File):- signal_eof_m_1(M,File).

signal_eof_m_1(M,File):- prolog_load_context(module,M),
   qdmsg(info(load_mpred_file_complete(M:File))),
   GETTER=t_l:eof_hook(WasM,File,TODO),
   forall(clause(GETTER,Body,Ref),(qdmsg(eof_hook(GETTER:-Body)),
...
call_on_eof(File,Call):- strip_module(Call,M,P),asserta(t_l:eof_hook(M,File,P)),
   must(M\==glogicmoo_util_with_assertions),
...
system:term_expansion(EOF,Pos,_,_):-nonvar(Pos),prolog_load_context(module,M),
  EOF==end_of_file,signal_eof_m(M)->fail.

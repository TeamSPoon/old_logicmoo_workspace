...
get_rtrace(X,Y):- tracing,!,'$leash'(Old, Old),'$visible'(OldV, OldV),rtrace, Y= call_cleanup(X,(('$leash'(_, Old),'$visible'(_, OldV),stop_rtrace))).
get_rtrace(X,Y):- '$leash'(Old, Old),'$visible'(OldV, OldV), (Y= call_cleanup(X,(('$leash'(_, Old),'$visible'(_, OldV))))).
...
loop_check(Call):- parent_goal(Term,2)->make_key(Term+Call,Key)-> loop_check_term(Call,Key,fail).
loop_check(Call, TODO):- parent_goal(Term,2)->make_key(Term+Call,Key)-> loop_check_term(Call,Key,TODO).
...
no_repeats_dif(Vs,Call):- dif(Vs,_), get_attr(Vs,dif,vardif(CONS,_)),!,
  Call,copy_term_nat(Vs,C),nb_linkarg(2, CONS, [_-C|CONS]).
...
no_repeats_old(Vs,Call):- CONS = [_], (Call), hotrace(( \+ memberchk_same(Vs,CONS), copy_term(Vs,CVs), CONS=[_|T], nb_linkarg(2, CONS, [CVs|T]))).
...
no_repeats_u(Vs,Call):- CONS = [_], (Call), hotrace((  CONS=[_|T],\+ memberchk_pred_rev(subsumes_term,Vs,T), copy_term(Vs,CVs), nb_linkarg(2, CONS, [CVs|T]))).
...
add_newvar(N,V):- ((b_getval('$variable_names', Vs), 
  (member(NN=Was,Vs),N==NN,var(Was),var(V),(Was=V))->true; nb_setval('$variable_names',[N=V|Vs]))),!.
...
nonvar_search(M:F/A):-nonvar(M),!,nonvar(A).

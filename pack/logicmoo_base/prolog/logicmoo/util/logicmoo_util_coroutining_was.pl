...
was_c_c_l(Unifier,OrNode) :-
	...
	was_c_c_l_aux(Unifier,OrNode,List,Tail).

extend_ornode(OrNode,N,List,Vars) :-
	( get_attr(OrNode,was,Attr) ->
		Attr = node(M,Vars),
		O is N + M - 1
	;
		O = N,
	...
	put_attr(OrNode,was,node(O,List)).

was_c_c_l_aux([],_,List,List).
was_c_c_l_aux([X=Y|Unifier],OrNode,List,Tail) :-
	List = [X=Y|Rest],
	add_ornode(X,Y,OrNode),
	was_c_c_l_aux(Unifier,OrNode,Rest,Tail).
...
add_ornode_var1(X,Y,OrNode) :-
	( get_attr(X,was,Attr) ->
		Attr = varwas(V1,V2),
		put_attr(X,was,varwas([OrNode-Y|V1],V2))
	;
		put_attr(X,was,varwas([OrNode-Y],[]))
	).

add_ornode_var2(X,Y,OrNode) :-
	( get_attr(Y,was,Attr) ->
		Attr = varwas(V1,V2),
		put_attr(Y,was,varwas(V1,[OrNode-X|V2]))
	;
		put_attr(Y,was,varwas([],[OrNode-X]))
	).

was:attr_unify_hook(varwas(V1,V2),Other) :-
	( var(Other) ->
		reverse_lookups(V1,Other,OrNodes1,NV1),
		or_one_fails(OrNodes1),
		get_attr(Other,was,OAttr),
		OAttr = varwas(OV1,OV2),
		...
		append(NV1,NOV1,CV1),
		append(NV2,NOV2,CV2),
                sort(CV1,CV1S),sort(CV2,CV2S),
		( CV1S == [], CV2S == [] ->
			del_attr(Other,was)
		;
			put_attr(Other,was,varwas(CV1S,CV2S))
		)
	;
...
or_one_fail(OrNode) :-
	( attvar(OrNode) ->
		...
		( NCounter == 0 ->
			fail
		;
			put_attr(OrNode,was,node(NCounter,Pairs))
		)
	;
		fail
	).
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   The attribute of a variable X is varwas/2. The first argument is a
...
was:attribute_goals(Var) -->
	(   { get_attr(Var, was, varwas(Ors,_)) } ->
	    or_nodes(Ors, Var)
	;   or_node(Var)
	).

or_node(O) -->
        (   { get_attr(O, was, node(_, Pairs)) } ->

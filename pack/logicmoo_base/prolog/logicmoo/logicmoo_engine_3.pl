...
nnf(_,_, _,Var,[Var] ,Var,1):- is_ftVar(Var),!.
...
simplify_cheap(IN,_):- var(IN),!,fail.
...
cf(KB, Orig,NNF, PNF, SET):-loop_check(cf0(KB, Orig,NNF, PNF, SET),trace).
cf0(KB, Orig,NNF, PNF, SET):-
 removeQ(KB, Orig,PNF,[], UnQ), 
  cnf(Orig,UnQ,CNF),  display_form(cnf:-CNF), clausify(KB,CNF,Cla,[]),!,
  flatten_or_list(Orig,CNF,Flat),make_clause_Vs_each(KB,Flat,Cla4),!,
  display_form(flat:-Flat),display_form(cla4:-Cla4),
  append(Cla,[cl([kbMark(Orig)],[])|Cla4],SO),!,
  maplist(correct_cls,SO,SOSOSO),
  list_to_set(SOSOSO,SET),!.
...
removeQ(KB, Orig, IN,FreeV,OUT):- for_nesc,fail, simplify_cheap(IN,MID), IN\=@=MID, removeQ(KB, Orig, MID,FreeV,OUT).

removeQ(KB, Orig, H, Vars, HH ):- convertAndCall(as_dlog,removeQ(KB, Orig,H, Vars, HH )).
removeQ(KB, Orig, n(Neg,n(Neg,F)),Vars, XF):- \+ leave_as_is(F), !, removeQ(KB, Orig,  F,Vars, XF) .

removeQ(KB, Orig, n(Neg,nesc(b_d(B,X), n(Neg,F))),Vars, XF):- for_nesc,!,removeQ(KB, Orig, poss(b_d(B,X), F),Vars, XF).
removeQ(KB, Orig, n(Neg,poss(b_d(B,X), n(Neg,F))),Vars, XF):- for_nesc,!,removeQ(KB, Orig, nesc(b_d(B,X), F),Vars, XF).

removeQ(KB, Orig, n(Neg,nesc(b_d(B,X), (F))),Vars, XF):- for_nesc,!,removeQ(KB, Orig, poss(b_d(B,X), n(Neg,F)),Vars, XF).
removeQ(KB, Orig, n(Neg,poss(b_d(B,X), (F))),Vars, XF):- for_nesc,!,removeQ(KB, Orig, nesc(b_d(B,X), n(Neg,F)),Vars, XF).

removeQ(KB, Orig, nesc(b_d(B,X), n(Neg,F)),Vars, XF):- for_nesc,!,removeQ(KB, Orig, n(Neg,poss(b_d(B,X), F)),Vars, XF).
removeQ(KB, Orig, nesc(b_d(B,X), n(Neg,F)),Vars, XF):- for_nesc,!,removeQ(KB, Orig, n(Neg,poss(b_d(B,X), F)),Vars, XF).
removeQ(KB, Orig, poss(b_d(B,X), n(Neg,F)),Vars, XF):- for_nesc,!,removeQ(KB, Orig, n(Neg,nesc(b_d(B,X), F)),Vars, XF).

removeQ(KB, Orig, all(X,F),Vars, HH):- !,  removeQ(KB, Orig,F,[X|Vars], RQ0),RQ0=HH.
removeQ(KB, Orig,  exists(X,F),Vars, HH):- skolem_setting(removeQ),!,
    wdmsg(removeQ(skolemizing(exists(X,F)))),
...
removeQ(KB, Orig, ':-'(H,B), Vars, ':-'(HH,BB ) ):- for_dnf,!, removeQ(KB, Orig,H, Vars, HH ),removeQ(KB, Orig,B, Vars, BB).
removeQ(_, _, cl(H,B), _, O ):- for_dnf, !,correct_cls(cl(H,B),O).
removeQ(KB, Orig,     [ H|B ],Vars, [ HH|BB ] ):- !,removeQ(KB, Orig,H, Vars, HH ),removeQ(KB, Orig,B, Vars, BB).


removeQ(KB,_Orig, F,_,FOO ):- for_dnf, fail,kb_nlit(KB,Neg),once(nnf(Neg,F,FO)),F\=@=FO, loop_check(removeQ(KB,_,FO,_,FOO)).

removeQ(KB,Orig, H,Vars,HH ):- H=..[F|ARGS],!,removeQ(KB,Orig, ARGS,Vars,ARGSO ),!,HH=..[F|ARGSO].
...
make_1_cl(KB,One,List,cl([One],NewBodyListO)):- delete_eq(List,One,Rest),
  maplist(negate_one(KB),Rest,NewBodyList),
...
skolem(KB, Orig, F, X, FreeV, Out):-  
   must(skolem_f(KB, Orig, F, X, FreeV, Sk)),
   must(Out=('=>'(skolemConstraint(X,Sk,F),F))),!.
...
kif_to_boxlog((HEAD:- BODY),KB,Why,FlattenedO):-  
  must_det_l((
   ...
   demodal_sents(HEADL,HEADLM),
   demodal_sents(BODYL,BODYLM),
   maplist(correct_cls,[cl(HEADLM,BODYLM)],NCFs),!,
...
kif_to_boxlog(WffIn,KB,Why,FlattenedO):-    
  must_det_l((
   ...
   pnf(NNF,PNF),!,
   %wdmsgl(pnf(PNF)),
   cf(KB,Orig,PNF,NCFsI0),!,
   demodal(NCFsI0,NCFsI),
...
boxlog_to_prolog((H , B),(HH , BB)):- !,boxlog_to_prolog(H,HH),boxlog_to_prolog(B,BB).
boxlog_to_prolog((H ; B),(HH ; BB)):- !,boxlog_to_prolog(H,HH),boxlog_to_prolog(B,BB).
boxlog_to_prolog(IN,OUT):- once(demodal_sents(IN,M)),IN\=@=M,!,boxlog_to_prolog(M,OUT).

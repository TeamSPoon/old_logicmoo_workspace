...
write_out_kif_tn(Assertion_Chars,computed(comment(_)),Vars,Ctx,Context,Maintainer):-
         fmt('Syntax Error: Unmatched parentheses in "~s"\n',[Assertion_Chars]).
...
clientEvent(Channel,Agent,Options):-
	...
	getMooOption(Agent,user='Web',Agent),
	getMooOption(Agent,interp='kif',Interp),
	fmt('<H3><Font Color=Red>Listing Disabled Assertions...</Font></H3>',[]),!,
...
show_disabled_assertions(Context,Ctx):-fmt('<H3><Font Color=Red>Done.</Font></H3>',[]).

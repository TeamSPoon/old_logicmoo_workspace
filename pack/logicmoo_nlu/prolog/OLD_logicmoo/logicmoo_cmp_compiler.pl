...
buildDeclaredIsaExtent:-
      ...
      fmt('~q.~n',[isNearestIsaCache(E,C)]),
      fail.
buildDeclaredIsaExtent:-
      decontextualizedHLFact(isNearestIsaCacheNG(E,C)),
      fmt('~q.~n',[isNearestIsaCacheNG(E,C)]),
      fail.
...
trimIsaCache:-
	 isNearestIsaCache(Isa,Collection),
      ...
      fmt('~q.~n',[isNearestIsaCache(Isa,Super)]),fail.
...
rememberHLFactNumbered(X):-fmt('~q.~n',[X]).
...
rewriteRuleOnA(HVExec,MtNext,Call,O):-compound(Call),!,Call=..List,(rewriteRuleOnAL(HVExec,MtNext,List,O);fmt(user_error,'~q.~n',[rewriteRuleOnAL(List,O)])),!.
...
writePercentAndTime(Format,SoFar,Total,NextPercent,NewPercent):-
				...
				fmt(Format,[NewPercent,TimeLeft]),!.
...
import_ado_cache(ContextName,Context):-
	fmt('~n% Done importing ~w.~n~n',[ContextName]),!.
...
show_template(Surface,Axiom,Result):-
	fmt('~nSurf: ~q~nResult:  ~q~n ~q~n~n',[Surface,Axiom,Result]),
...
list_clause_out(cl([H],[])):-fmt('fact(~q).~n',[H]).
list_clause_out(cl([],H)):-fmt('disj(~q).~n',[H]).
list_clause_out(H):-fmt('~q.~n',[H]).
...
cmp_nl:-nl.
cmp_format(X):-fmt(X).
cmp_format(X,Y):-fmt(X,Y).
cmp_format(S,X,Y):-fmt(S,X,Y).

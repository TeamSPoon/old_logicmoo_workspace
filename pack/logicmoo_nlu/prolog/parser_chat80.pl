...
test_chat80_regressions:- w_tl(tracing80,with_no_assertions(thglobal:use_cyc_database,forall(must_test_80(U,R,O),process_run_diff(report,U,R,O)))).

:-export(test_chat80_sanity/0).
test_chat80_sanity:- w_tl(tracing80,with_no_assertions(thglobal:use_cyc_database,forall(must_test_80_sanity(U,R,O),process_run_diff(report,U,R,O)))).
           ...
           w_tl(t_l:chat80_interactive,
            with_no_assertions(t_l:useOnlyExternalDBs,
             with_no_assertions(thglobal:use_cyc_database,
                  w_tl(usePlTalk, (told, repeat, prompt_read('CHAT80> ',U),  

...
test_chat80_regressions:- with_assertions(tracing80,with_no_assertions(thglobal:use_cyc_database,forall(must_test_80(U,R,O),process_run_diff(report,U,R,O)))).

:-export(test_chat80_sanity/0).
test_chat80_sanity:- with_assertions(tracing80,with_no_assertions(thglobal:use_cyc_database,forall(must_test_80_sanity(U,R,O),process_run_diff(report,U,R,O)))).

...
detatch_object(Obj):-  
  (call_u(mudPossess(Agent,Obj))->clr(mudPossess(Agent,Obj));true),
  (call_u(mudAtLoc(Obj,LOC))-> clr(mudAtLoc(Obj,LOC));true),
  (call_u(localityOfObject(Obj,R))-> clr(localityOfObject(Obj,R));true),
  (clr(inRegion(Obj,_))),!.

...
http_server(Port, Pending):-
	...
	bind(Soc, _/Port),
	listen(Soc, Pending),
	block(loop(Soc),int,Interrupted=true),
	close(Soc), !,
	(Interrupted==true -> exit_block(int) ; fail).
...
loop(Soc):-
	accept(Soc, _, S), !,
	(block(process(S), Tag, true) -> true ; true),
...
process(S):-
	...
	block(
		...
		int,
		(Output="", Status=400, RespHttpParams=[], Interrupted=true)
		),
	...
	(nonvar(Interrupted) -> exit_block(int) ; true).
...
parse_Params([H|T], [P|TT]):-
	...
	phrase(header(P), L, _), !,
	parse_Params(T, TT).
% silently ignore header fields that are not in the grammer
parse_Params([_|T], TT):-
	parse_Params(T, TT).

...
xml_parse( Chars, Document, Controls ) :-
	...
	; prolog:illarg(var, Goal, 0)
	), !. /*MC*/

xml_options(L, _, _, Goal, ArgNo) :- 
	var(L), !,
	prolog:illarg(var, Goal, ArgNo).
xml_options([], Opt, Opt, _, _) :- !.
xml_options([X|L], Opt0, Opt, Goal, ArgNo) :- !,
	(   xml_option(X, Opt0, Opt1) -> true
        ;   prolog:illarg(domain(term,xml_parse_option), Goal, ArgNo, X)
        ),
	xml_options(L, Opt1, Opt, Goal, ArgNo).
xml_options(L, _, _, Goal, ArgNo) :- 
	prolog:illarg(domain(term,list), Goal, ArgNo, L).

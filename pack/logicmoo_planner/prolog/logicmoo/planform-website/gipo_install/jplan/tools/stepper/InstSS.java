...
/**
 * to show the oclSS for instantiation with mouse clicking.
 */
public class InstSS extends javax.swing.JDialog {
    ...
    private List objectList = new ArrayList(); //the whole object list from the curDom
    private OPredicate.pArg selectedPArg;
    private JComboBox combo;

  /** 
   * Creates an instance of the InstSS dialog window
   * @param curDom oclDomain
   * @param curSS - method a reference method
   * @param parent parent frame
   */
    public InstSS(oclDomain curDom, oclSS curSS, HStepperCanvas parent) {
	super(parent.getTheParent().getTheParent(), "State Instantiation Window"); 
	setModal(true);
	this.parent = parent;
	this.curDom = curDom;
	this.curSS = curSS;
	this.objectList = curDom.objects; //lists of oclObject
	briefAreaPred = new oclPredicate("Achieve");
	initComponents ();
	pack ();
	setSize(300, 200);
    }
	...
	briefAreaPred = curSS.getPredExpression(curDom);    /* WZ 11/6/02 */
	...
	briefArea.addMouseListener(new MouseAdapter(){
		...
		if(me.getModifiers() == MouseEvent.BUTTON3_MASK) {
			...
			if (popupMenu.getComponentCount() > 0) {
			    ...
			    Utility.debugPrintln("No object items in the popup menu.");
			}
     ...
     * 
     */
    private void showSSDetail(){
	StringWriter opDetail = new StringWriter();
	curSS.oclPrintComponent(new PrintWriter (opDetail) ,0,false);
	...
	combo.addActionListener (new java.awt.event.ActionListener () {
		...
		try {
		    ...
		    briefArea.setText(briefAreaPred.toString());
		    curSS.replaceVariableName(selectedPArg, objname);
		    showSSDetail();
		    popupMenu.hide(); 
		} catch (Exception e) {
     ...
     * @param parent
     */
    public static oclSS showInstantiation(oclDomain curDom, oclSS ss, HStepperCanvas parent){
	try {
	    oclSS oclss = (oclSS)ss.clone();
	    InstSS pw = new InstSS(curDom, oclss, parent);

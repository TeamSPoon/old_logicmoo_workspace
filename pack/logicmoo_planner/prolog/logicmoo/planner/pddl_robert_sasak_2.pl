...
try_solve(D,P,S):- once(time_out(solve(D, P, S), 1000, Result)), Result == time_out, portray_clause(hard_working:-try_solve(D,P,S)),fail.
...
%term_to_ord_term([H|T], R):- R = [H|T],!.
term_to_ord_term([H|T], R):-
    term_to_ord_term(H, OH),
    term_to_ord_term(T, OT),
    ord_add_element(OT, OH, R),
    (([H|T]\=@= R) *->
    (!,nop(portray_clause(term_to_ord_term:-term_to_ord_term0([H|T], R)))); true).
...
parseDomain(File, Output, R) :-  
   ...
   %  (nonvar(Output)->true;empty_assoc(Output)),
    ...
    domainBNF(PROPS,Output, List, R),!.

set_nb_propval(PROPS,Name,ValueIn):-
  term_to_ord_term(ValueIn,Value),
  arg(1,PROPS,Dict),
  (get_dict(Name,Dict,Old)->
    ((merge_values(Old,Value,Set),(Set\==Old -> nb_set_dict(Name,Dict,Set) ; true)));
  (put_dict(Name,Dict,Value,NewDict),nb_setarg(1,PROPS,NewDict))).

merge_values([],Value,Value).
merge_values(Old,Value,Value):-Old==Value.
merge_values(Old,Value,New):-is_list(Old),is_list(Value),ord_union(Old,Value,New).
merge_values(_,Value,Value):-
...
sterm2pterm(VAR,'$VAR'(UP)):-svar(VAR,SVAR),!,upcase_atom(SVAR,UP).
...
sterm(PROPS,_) --> [')'],{!,fail}.
sterm(PROPS,[]) --> ['(',')'],!.
sterm(PROPS,A) --> action_def(A),!.
sterm(PROPS,requires(R)) --> require_def(R),!.
sterm(PROPS,types(L))                    --> ['(',':',types],      typed_list(name, L), [')'].
sterm(PROPS,constants(L))                --> ['(',':',constants],  typed_list(name, L), [')'].
sterm(PROPS,predicates(P)) --> predicates_def(P).
sterm(PROPS,[H,T]) --> skey(H),['('],!,zeroOrMore(sterm(PROPS), T), [')'],!,set_nb_propval(RPOPS,
%sterm(PROPS,[H,T]) --> ['('],skey(H),!,oneOrMore(sterm(PROPS), T), [')'],!.
sterm(PROPS,[H|T]) --> ['('],satom(H),!, zeroOrMore(sterm(PROPS), T), [')'],!.
sterm(PROPS,[H|T]) --> ['('],sterm(PROPS,H),!, zeroOrMore(sterm(PROPS), T), [')'],!.


sterm(PROPS,N)-->satom(N).
...
%
domainBNF_dcg(PROPS,domain(N, R, T, C, P, F, C, S))
                        --> ['(','define', '(','domain'], name(N), [')'],
                        dcgMust(domainBNF_rest( PROPS, domain(N, R, T, C, P, F, C, S))),
                        {set_nb_propval(PROPS,domain_name,N)}.
                        
                        % R, T, C, P, F, C, S
domainBNF_rest( PROPS,domain(_N, R, T, C, P, F, C, S) ) --> 
...
%                            dcgMust (constraints(C)   ; []),    %:constraints
                            ...
                            set_nb_propval(PROPS,functions,F), %:fluents
...
problem_dcg(problem(Name, Domain, R, OD, I, G, UNK, MS, LS))   
                                --> ['(',define,'(',problem,Name,')',
                                     '(',':',domain, Domain,')'],
...
%parse_file(+File).
test_parse_file(F,O):- must(read_file(F, L)),!,((domainBNF(PROPS,O, L, _R1); must(((problem(O, L, _R2)))))),!.

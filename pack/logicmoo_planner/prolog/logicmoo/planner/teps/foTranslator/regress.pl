...
compute_new_domain(Goal,Type) :-
	...
	writef("\n\n CALLING FINAL GOAL WITH:\n %w",[[QuantifierVars,QuantifierPrefix,AutList,UnInstGoal,VarList,1]]),
	add_final_goal(QuantifierVars,QuantifierPrefix,AutList,UnInstGoal,VarList,1). 
...
aut2newfluents(Quant,Aut,AutNumber,Type) :-
	...
	forall((member([Id,_,_,_],Aut),id2fluentvar(Id,AutNumber,VList,Fluent)),myassertz(new_fluent(Fluent))),
	(Type=dp -> %aditional prev_autstate predicates are declared
	    forall((member([Id,_,_,_],Aut),
		    id2prevfluentvar(Id,AutNumber,VList,Fluent)),
		   myassertz(new_fluent(Fluent)));
	    true
	).

add_final_goal([],[],[],Goal,[],_) :- recursive_variable_subst(Goal,Goalp),myassertz(new_goal(Goalp)).

add_final_goal([QVar|QVList],[Quant|QList],[Aut|AList],UnInstGoal,[Var|VList],N) :-
	writef("I am here!!\n"),
	aut2goal(QVar,Quant,Aut,N,Var),
	N1 is N+1,
	add_final_goal(QVList,QList,AList,UnInstGoal,VList,N1).

aut2goal(VList,Quant,Aut,AutNumber,GoalFla) :-
...
%%% The following function would need to be reimplemented for the first-order
%%% version
%aut2initial(_,Aut,AutNumber,cr) :- %Quant parameter not implemented here
%	findall(Fluent,(member([_,_,1,AdjList],Aut),
...
% constant_vector(+N,?C)
% 
% Unifies C with a vector of constants of length N
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% add_new_initial(List)
%%
%% Facts in List will be added to the initial state
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% The following function would need to be reimplemented
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% new_effects(Quant,IdPredList,AutNumber,dp)
%%
%% Given that IdPredList contains a list of states and its predecessors,
%% generates a set of effects for each of the new predicates in the
%% domain that represent the states of the automata.
%% Variable Quant contains the quantifier prefix of the formula
...
new_effects(Quant,[[Id,PredList]|Rest],AutNumber,dp) :-
	...
	(DefSSS \== false ->
	    myassertz(derived(F,DefSSS))	%%derived predicate definition
	;
	    true
	),
	forall(action(A),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% quantifier_prefix(+Fla,?QList,?QVars,?FnoQ)
%%
%% Given a formula with quantifiers in Fla, unifies:
%% - QList with a list of quantifiers and variables in the prefix of Fla
%% - QVars with the list of variables in the prefix of Fla
%% - FnoQ with the formula without the quantifier prefix
...
%%
%% Unifies V with a list of varibles of the same length as L

...
sendRcxMessage(Msg, End, Reply) :-
    ...
    putRcxMess(Msg),
    getRcxMess(Reply, End2).

sendRcxMessage(Msg, End, Reply) :-     % Here if getRcxMess above failed
    currentTime(Now),
...
receiveRcxActionNumber(Actions) :-
    ...
    ((rcxNoExogMessage(Ans), Actions = []) ;  % RCX reports none
     (getEncodedNumber(Ans, action, End2, N), Actions = [N])). % RCX returns action

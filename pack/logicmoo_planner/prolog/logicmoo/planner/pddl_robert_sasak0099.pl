...
try_solve(D,P,S):- portray_clause(hard_working(0):-try_solve(D,P,S)),fail.
% try_solve(D,P,S):- once(time_out(solve(D, P, S), 2000, Result)), Result == time_out, portray_clause(hard_working(3):-try_solve(D,P,S)),fail.
try_solve(D,P,S):- gripe_time(8,time_out((solve(D, P, S)), 9000, Result)),!, % time limit for a planner (was 500000)
   ((\+ is_list(S)
...
is_pred_like_block(block).
is_pred_like_block(table).
...
term_to_ord_term(Term, R):-t2ot(Term, OT),!,t2ot_r(OT, R).

t2ot_r(A, A):- \+ compound(A), !.
t2ot_r(HT, R):- HT =.. [H,T],is_list(T),is_pred_like_block(H), HTT =.. [H|T],!,t2ot_r(HTT, R).
t2ot_r(HT, R):- is_list(HT),!,maplist(t2ot_r,HT, R).
t2ot_r(HT, R):- HT=..[H|T],!,maplist(t2ot_r,T,TR),R=..[H|TR].
t2ot_r(R, R).
...
get_actions(     	domain(_, _, _, _, _, _, _, A, _NP), A).    
get_problem_name(	problem(N, _, _, _, _, _, _, _, _, _NP), N).
get_init(    problem(_, _, _, _, I, _, _, _, _, _NP), I).
get_goal(    problem(_, _, _, _, _, G, _, _, _, _NP), G).
get_metric(    problem(_, _, _, _, _, _, _, M, _, _NP), M).
get_objects(    problem(_, _, _, O, _, _, _, _, _, _NP), O).
...
domainBNF_dcg(Struct, Struct)
                        ...
                        {must(Struct = domain(N, R, T, C, P, F, C, S, OptionsDict)),(var(OptionsDict)-> rb_new(OptionsDict) ; true)},
...
%                           set_nb_propval(Struct,constraints,C)   ,    %:constraints
...
problem_dcg(Struct)   
                                --> ['(',define,'(',problem,Name,')',
                                     '(',':',domain, Domain,')'],
   {must((Struct = problem(Name, Domain, R, OD, I, G, UNK, MS, LS, OptionsDict))),

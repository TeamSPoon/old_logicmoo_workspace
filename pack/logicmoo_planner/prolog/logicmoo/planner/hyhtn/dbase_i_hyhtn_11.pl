...
:-include(planner_header).  % TLMc  3/99
...
:- dynamic node/7,node/5,tp_node/6,tn/6,solved_node/5.
:- dynamic produce/4,goal_related/1,achieved_goal/1.
:- dynamic solved_node/2.
:- dynamic closed_node/6.

% Domain Env
:- dynamic methodC/7, operatorC/5.
:- dynamic gsubstate_classes/3.

% Ground for Task
:- dynamic gsstates/3,gpred/2, gpredicates/2.

% Contents of a OCLh Domain
% ,call(dynamic),call(multifile)
:-env_learn_pred([g,dom],[sorts/2,
  objects/2,
  predicates/1,
  atomic_invariants/1,
  substate_classes/3,
...
:- env_retractall((initial_node/_,final_node/_, node/_,node/_,tp_node/_,tn/_,solved_node/_,
   produce/_,goal_related/_,achieved_goal/_,
   solved_node/_,
   closed_node/_,
   gsubstate_classes/_,
   gsstates/_,gpred/_)).
...
op_num(0).
set_op_num(N):-remove_op_num,assert(op_num(N)),!.
get_op_num(N):-assertion(op_num(N)),!.
get_op_num(0):-!,assert(op_num(0)).
remove_op_num :- retractall(op_num(_)).
incr_op_num:- retract(op_num(N)), N1 is N+1, assert(op_num(N1)),!.
incr_op_num:- assert(op_num(1)).
:-set_op_num(0).

% gensym_special(Root,Atom) :-!, gensym(Root,Atom).
...
gensym_special(Root,Atom) :-
     ...
     name(Atom,Name),!.
gensym_special(Root,Atom) :- getnum(Root,Num), atom_concat(Root,Num,Atom).

incnxtnum(Root):-!, getnum(Root,_Num),!.
% incnxtnum(F):-flag(F,N,N+1).

:- dynamic current_num/2.

% returns 1 or last value +1 
% getnum(F,NN):- !,E=N+1,flag(F,N,E),NN is E.
getnum(Root,Num) :-
...
% getnum(F,N):- incnxtnum(F),current_num(F,N),!.

% peek_num(F,N):- nonvar(F),current_flag(F),flag(F,N,N).
peek_num(F,N):- current_num(F,N).

set_num(F,N):- /*flag(F,_,N),*/ 
  retractall(current_num(F,_)),
  asserta(current_num(F,N)),!.

remove_all_numbs:- retractall(current_num(_,_)),!.
remove_num(F):- retractall(current_num(F,_)),!. %  ( nonvar(F) -> flag(F,_,0);  (current_flag(F),\+ atom_concat('$',_,F),flag(F,_,0),fail)).
remove_num(_).
...
:- dynamic((htn_task/3, planner_task/3)).
:- multifile htn_task/3, planner_task/3.


%  ss class (ss) expressions
%  invariants:
%     atomic invariants
%     -ve invariants
%     +ve invariants
...
get_htn_task(Id,Goal,Init):-htn_task(Id,Goal,Init).

%:-set_prolog_flag(unknown,fail).
solve(Id):- 
      dmsg(solve_htn_task(Id)),
      solve_htn_task(Id).

% solve(Id): -debug,trace,solve_htn_task(Id).
solve_htn_task(Id) :-
     get_htn_task(Id,Goal,Init),
     once((
    dmsg(('TASK'(Id):-Goal)),
...
clean_domain:-
     env_g_retractall(gpredicates(_,_)),
     env_g_retractall(gsubstate_classes(_,_,_)),
     env_g_retractall(methodC(_,_,_,_,_,_,_)),
     env_g_retractall(operatorC(_,_,_,_,_)).

clean_task:-
     ignore(remove_op_num),     
     remove_all_numbs,
     env_l_retractall(node(_,_,_,_,_)),
     env_l_retractall(tn(_,_,_,_,_,_)),
     env_l_retractall(goal_related(_)),
     env_l_retractall(produce(_,_,_,_)),
     env_l_retractall(solved_node(_,_)),
     env_l_retractall(tp_node(_,_,_,_,_,_)),
     env_l_retractall(closed_node(_,_,_,_,_,_)),
     env_l_retractall(score_list(_)),
     set_op_num(0),
     assert(my_stats(0)),!.
...
planner_interface(G,I,SOLN,NODES):-
	...
	find_relate_state,
        retract(op_num(_)),
        assert(op_num(0)),
        statistics(runtime,[_,Time1_0]),Time is Time1_0 + 0.0001,
...
planner_interface_borkws(G,_I,_SOLN,_NODES):-
     tell(user),nl,
     dmsg('failure in initial node'(planner_interface(G))),dmsg('failure '),nl,nl,!.
...
start_solve(_SOLN,_NODES):- fail,
     kill_file(Kill),
     file_exists(Kill),
	write('Found kill file'),nl,fail.

start_solve(Sol,OPNUM):-
     env_l_retract(final_node(Node)),
     remove_all_numbs,
     getN_statics(Node,Statics),
     statics_consist(Statics),
     extract_solution(Node,Sol,SIZE),
     statistics(runtime,[_,CP__0]),CP is CP__0 + 0.0001,
     TIM is CP/1000,
...
assert_node(Name,Pre,Decomp,Temp,Statics):-
     ...
     env_l_assert(final_node(node(Name,Pre,Decomp,Temp,Statics))),!.
assert_node(Name,Pre,Dec,Temp,Statics):-
     gensym_special(root,SYM),
     env_l_assert(node(SYM,Pre,Dec,Temp,Statics)),!.
...
expand_hp(HPid,TN,ACH,Pre,Post,State,Statics,Statics1):-
     ...
     env_l_assert(tp_node(TP,Pre,Post,Statics,Score,Steps)),
...
get_all_related_goal(Goal):-
     env_l_retractall(goal_related(_)),
     env_l_assert(goal_related(Goal)),
     get_all_related_goal1(Goal,States),
     env_l_retractall(goal_related(_)),
     env_l_assert(goal_related(States)),!.
...
get_all_related_goal1([se(Sort,Obj,SE)|Rest],States):-
     ...
     env_l_retract(goal_related(List)),
     set_append_e(List,OPre,List1),
     env_l_assert(goal_related(List1)),
     fail.
...
fwsearch(TN,ST,State):-
     env_l_retract(solved_node(ST,step(HP,Name,Pre,State,exp(TN)))).
fwsearch(TN,ST,State):-
     select_tnode(tp_node(TP,Pre,Post,Statics,Score,Dec)),
     env_l_assert(closed_node(TP,Pre,Post,Statics,Score,Dec)),
...
select_tnode(tp_node(HPid,Pre,Post,Statics,Score,Dec)) :-
     env_l_retractall(score_list(LS)),
     env_l_assert(score_list([])),
     lowest_score(Score),
     env_l_retract(tp_node(HPid,Pre,Post,Statics,Score,Dec)),!.
%     tell(user),nl,write('new level'),nl,told,!.

% find the lowest_score of tp_node
lowest_score(LScore):-
     tp_node(HPid,Pre,Post,Statics,Score,Dec),
     env_l_retract(score_list(LS)),
     env_l_assert(score_list([Score|LS])),
     fail.
lowest_score(LScore):-
     env_l_retract(score_list(D)),
     sort(D,[LScore|SD]).

% :- style_check(-discontiguous).

:- discontiguous(assert_tnode/6).
...
assert_tnode(TP,Pre,Post,Statics,Score1,Dec):-
     ...
     env_l_assert(solved_node(Statics,OneDec)),!.
...
assert_tnode1(TP,Pre,Post,Statics,Score,ExpDec,UnexpDec):-
     ...
     env_l_assert(tp_node(TP1,Pre,Post,Statics,Score1,Dec)),!.
...
combine_steps1(step(HP1,Name1,Pre1,Post1,exp(TN1)),step(HP2,Name2,Pre2,Post2,exp(TN2)),step(HP,achieve(Post2),Pre1,Post2,exp(TN))):-
     ...
     env_l_assert(tn(TN,achieve(Post0),Pre0,Post0,[before(TN1,TN2)],[step(HP1,Name1,Pre1,Post1,exp(TN1)),step(HP2,Name2,Pre2,Post2,exp(TN2))])),!.
...
%when nec
assert_related_states1(A,Pre,[se(Sort,Obj,SE)|Post],ST):-
     env_l_assert(produce(se(Sort,Obj,SE),A,Pre,ST)),
     assert_related_states1(A,Pre,Post,ST),!.
...
assert_related_states21(A,Pre,[sc(Sort,Obj,SE=>SS)|Trans],ST):-
     ...
     env_l_assert(produce(se(Sort,Obj,SSR),A,Pre1,ST2)),
...
% (se to ss)
make_into_ss([],[]):-!.
make_into_ss([H|T],Out):- make_into_ss(H,HL),make_into_ss(T,TL),flatten([HL,TL],OutL),!.
make_into_ss(se(Sort,Obj,Post),ss(Sort,Obj,Post)):-!.
make_into_ss(ss(Sort,Obj,Post),ss(Sort,Obj,Post)):-!.
make_into_ss(O,O).
...
select_node(node(Name,Pre,Temp,Decomp,Statics)) :-
     env_l_retract(node(Name,Pre,Temp,Decomp,Statics)),
...
statics_consist_instance0(Invs,[ne_back(A,B)|TStatics]):- dif(A,B),
     not(A==B),
     statics_consist_instance0(Invs,TStatics).
statics_consist_instance0(Invs,[ne(A,B)|TStatics]):- dif(A,B),
...
statics_consist1(Invs,[ne_back(A,B)|TStatics]):- dif(A,B),
     not(A==B),
     statics_consist1(Invs,TStatics),!.
statics_consist1(Invs,[ne(A,B)|TStatics]):- dif(A,B),
     append(TStatics,[ne_back(A,B)],TStatics1),
...
change_op_representation :-    
     ...
     env_g_assert(methodC(A,PreR,PostR,Statics2,T1,achieve(ACH),Dec1)),
     fail.
change_op_representation :-
     operator(A,B,C,D),
     make_ss_to_se(B,B0),
...
%      make_sc_primitive(D,D1),
	%can't do that because it narrow the conditional change
     ...
     statics_consist(Statics),     
     env_g_assert(operatorC(A,PreR,PostR,D,Statics)),
     fail.
change_op_representation:-
     remove_num(sm),!.
...
%      tell(user),nl,write(tn(TN,Name,Pre1,Post1,Temp,Dec)),nl,told,
     env_l_assert(tn(TN,Name,Pre1,Post1,Temp,Dec)),!.
...
collect_grounded_pred:-
     ...
     env_g_retractall(gpred(Pred,_)),
     env_g_assert(gpredicates(Pred,GPredls)),
     fail.
collect_grounded_pred.
...
assert_subclass([HS|TS],Obj,Substate):-
     env_g_assert(gsstates(HS,Obj,Substate)),
     assert_subclass(TS,Obj,Substate).

collect_grounded_substates:-
     gsstates(Sort,Obj,_),
     setof(SStates,gsstates(Sort,Obj,SStates),GSStates),
     env_g_retractall(gsstates(Sort,Obj,_)),
     all_combined(GSStates,GSStates0),
     env_g_assert(gsubstate_classes(Sort,Obj,GSStates0)),
     fail.
collect_grounded_substates.
...
assert_gpred0(GPred,UPred):-
     ...
     env_g_assert(gpred(UPred,GPred)),
     fail.
assert_gpred0(GPred,UPred).
...
:-['sdom4.htns'],['tasks_sd4.htnt'].


solve40:-forall(solve(_),true).
/*
:-['ty_spec.ocls'],['ty_tasks.oclt'].
:-['r3_spec.ocls'],['r3_tasks.oclt'].

:-['sdom1.htns'],['tasks_sd1.htnt'].
:-['sdom2.htns'],['tasks_sd2.htnt'].
:-['sdom3.htns'],['tasks_sd3.htnt'].
*/

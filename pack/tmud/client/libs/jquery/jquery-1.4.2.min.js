...
q.getAttributeNode("id").nodeValue===m[1]?[q]:w:[]};n.filter.ID=function(m,q){var p=typeof m.getAttributeNode!=="undefined"&&m.getAttributeNode("id");return m.nodeType===1&&p&&p.nodeValue===q}}l.removeChild(g);l=g=null})();(function(){var g=s.createElement("div");g.appendChild(s.createComment(""));if(g.getElementsByTagName("*").length>0)n.find.TAG=function(h,l){l=l.getElementsByTagName(h[1]);if(h[1]==="*"){h=[];for(var m=0;l[m];m++)l[m].nodeType===1&&h.push(l[m]);l=h}return l};g.innerHTML="<a href='jquery-1.4.2.min.js#'></a>";
...
// jQuery plugin: JSON
// Taken from: http://www.ramirezcobos.com/2009/12/30/json-jquery-plugin/
jQuery.JSON={useHasOwn:({}.hasOwnProperty?true:false),pad:function(n){return n<10?"0"+n:n;},m:{"\b":'\\b',"\t":'\\t',"\n":'\\n',"\f":'\\f',"\r":'\\r','"':'\\"',"\\":'\\\\',"{":'\\x7b',"}":'\\x7d',"[":'\\x5b',"]":'\\x5d',",":'\\x2c',":":'\\x3a'},encodeString:function(s){if(/["\\\x00-\x1f{}\[\],:]/.test(s)){var mm=this.m;return'"'+s.replace(/([\x00-\x1f\\"{}\[\],:])/g,function(a,b){var c=mm[b];if(c){return c;}

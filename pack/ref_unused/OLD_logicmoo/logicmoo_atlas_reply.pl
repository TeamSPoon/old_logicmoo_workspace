...
sendError(Channel,Agent,Ref,Msg,UMap):-
      ...
      fmt(Channel,Agent,map([serialno:int(LocalSerial),refno:int(Ref),from:string(''),to:string(Channel),seconds:float(S1970),future_seconds:float(0.0),time_string:string(TimeString),args:list([map([message:string(Msg)]),Map]),parents:list(["error"]) ,id:string(''),objtype:"op",name:string('')])).

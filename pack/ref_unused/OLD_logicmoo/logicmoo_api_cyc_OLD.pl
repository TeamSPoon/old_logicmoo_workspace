...
% mud defined  is_string(X):-string(X),!.
% mud defined  is_string([A,B|_]):-integer(A),A>12,A<129,integer(B),B>12,B<129.
...
cycQueryResults(SocketId,InStream,OutStream,CycL,Mt,CycLified,Mtified,Vars,Result):-
      repeat,
      peek_code(InStream,PCode), % isDebug(fmt('PCODE (~q)~n',[PCode])),
...
cycQueryEachResult(__,InStream,Vars,Result,more):-
      readCycL(InStream,Trim), %isDebug(fmt('"~s"~n',[Trim])),
      getSurfaceFromChars(Trim,Result,_),!.%isDebug(fmt('~q.~n',[Result])),
%      syncCycLVars(Result,Vars),!.
...
%====================================================================
% mud defined trim(X,Y):-ltrim(X,R),reverse(R,Rv),ltrim(Rv,RY),reverse(RY,Y),!.
...
getWordTokens([],[]):-!.
getWordTokens([C|T],O):- C<33,!,getWordTokens(T,O).
getWordTokens([C|List],[Token|TList])  :- 
  get_token(C,List,Token,Rest),!,
  getWordTokens(Rest,TList).
...
mbkb:-saved(P,'#$BaseKB',A,C),functor(P,F,Arity),functor(PP,F,Arity),
      ...
      fmt('~q.~n',[saved(P,Mt,A,C)]),retract(saved(P,'#$BaseKB',A,C)),fail.
...
removeUniversalDuplicates:-saved(P,'#$UniversalVocabularyMt',A,C),functor(P,F,Arity),functor(PP,F,Arity),
      ...
      fmt('~q.~n',[saved(P,Mt,A,C)]),retract(saved(P,'#$UniversalVocabularyMt',A,C)),fail.
...
strFix(O,ListO):-atom_codes(O,C),!,getWordTokens(C,ListO),!.

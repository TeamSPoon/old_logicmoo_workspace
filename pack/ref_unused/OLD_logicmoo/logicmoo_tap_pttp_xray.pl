...
pttp_configuration :-
	true,fmt('PTTP CONFIGURATION:'),
	(count_beliefs_pred(true) ->
	    fmt('PTTP counts no beliefs.');
	    fmt('PTTP counts beliefs!')),
        (trace_search_progress_pred(nop) ->
	    fmt('PTTP does not true pttp_search progress.');
	    fmt('PTTP traces pttp_search progress!')),
	(compile_explaination_printing ->
	    fmt('PTTP compiles explaination printing!');
	    fmt('PTTP does not compile explaination printing.')),
	(compile_complete_search ->
	    fmt('PTTP compiles complete pttp_search!');
	    fmt('PTTP does not compile complete pttp_search.')).
...
%%% Lemma fmt indicates the usage of unit or disjunctive lemmas
...
write_ctheory(File,Context) :-
	...
	writeSTDERR(ContextFile),writeSTDERR(' written '),writeSTDERR(ContextFileSize),fmt(' bytes'),
	writeSTDERR(QFile), writeSTDERR(' written '),writeSTDERR(QFileSize), fmt(' bytes'),
	!.
...
%	fmt(unit_extraction),display_matrix(Matrix),display_matrix(M1),display_units(Units1),
	dp(M1,Model1),

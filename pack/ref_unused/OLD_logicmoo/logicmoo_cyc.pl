...
%
%  isMooOption(Var,Value). - tests for option
%
% ===================================================================
...
isMooOption(Name):-isMooOption(Name,on).
isMooOption(Name):-isMooOption(Name,yes).
isMooOption(Name=Value):-isMooOption(Name,Value).

isMooOption(Name,Value):-getCycOption_thread(Name,Value).
...
getCycConnection3(SocketId,OutStream,InStream):- 
      ignore(once(isMooOption(cycServer,Server))),
...
getCycConnection(Server,SocketId,OutStream,InStream):-
      ...
      thread_self(Thread),
      dmsg('Thread ~w Connected to Cyc TCP Server {~w,~w}\n',[Thread,InStream,OutStream]),
...
% Lowlevel printng
% ===================================================================
fmt(X,Y,Z):-catch((format(X,Y,Z),flush_output_safe(X)),_,true).
fmt(X,Y):-catch((format(X,Y),flush_output),_,true).
fmt(X):- once((atom(X) -> catch((format(X,[]),flush_output),_,true) ; fmt('~q~n',[X]))).
...
%streamClear(InStream) :-get_code(InStream, Was),((Was == -1) -> (true);(dmsg('FoundMore ~c ~q ~n',[Was,Was]),streamClear(InStream))),!.
...
defaultAssertOptions(Opts):-isMooOption(defaultAssertOptions,Opts).
...
queryParams(Backchain,Number,Time,Depth):-
   ignore(isMooOption(query(backchains),Backchain)),
   ignore(isMooOption(query(number),Number)),
   ignore(isMooOption(query(time),Time)),
   ignore(isMooOption(query(depth),Depth)),!.
...
makeConstant(_Const):-not(isMooOption(makeConstant)),!.
%makeConstant(_Const):-!.
...
user:exception(undefined_predicate, Pred ,retry):- isMooOption(hookCycPredicates,true),cycDefineOrFail(Pred).
...
%isCycLTerminationStateChar(C,C):- dmsg('Not terminal ~c ~q ~n',[C,C]),!,fail.
...
getSurfaceFromToks(WFFClean,OUT,VARSOut):- OUT=[unk_comment,WFFClean], dmsg('getSurfaceFromToks: ~q ~n',[OUT]),sleep(2),!.
...
fmt(X,Y,Z):-catch(format(X,Y,Z),_,true).
fmt(X,Y):-format(X,Y).
fmt(X):-format(X,[]).
...
processRequest(X):-catch(user:processRequestHook(X),E,( dmsg('processRequestHook: "~q" \n',[X:E]), fail)),!.
...
%dmsg([-1]).
%dmsg([[-1]]).
%dmsg(T):- isMooOption(opt_debug=off),!.
...
%dmsg(C,T):- isMooOption(opt_debug=off),!.
dmsg(_,F):-F==[-1];F==[[-1]].
dmsg(F,A):-
        nl(user_error),
        fmt(user_error,F,A),
...
sendNote_1(To,From,Subj,Message,Vars):- isMooOption(client=console),!, % In CYC
            toMarkUp(cycl,To,Vars,STo),
            toMarkUp(cycl,From,Vars,SFrom),
            toMarkUp(cycl,nv(Subj),Vars,SS),
            toMarkUp(cycl,nv(Message),Vars,SA),
...
writeCycEvent(_,_,_):-isMooOption(disp_hide_all=true),!.
writeCycEvent(_,_,_):-telling_file,!.
writeCycEvent(Class,_,_):-isMooOption(Class=false),!.
writeCycEvent(Class,_,_):-isMooOption(disp_explicit=true),not(isMooOption(_Class=true)),!.
...
writeCycEvent(Class,Message,Vars):-not(isMooOption(client=html)),!, toMarkUp(cycl,[Class,Message],Vars,Chars),write(Chars),nl.
writeCycEvent(Class,Message,Vars):-isMooOption(client=html),!, event_to_chars(leml,Class,_Message,Vars,Chars),write(Chars),nl.

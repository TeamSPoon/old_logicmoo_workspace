...
writeMooMenu:-
         fmt('ask - switches to ask mode \ntell - switches back to tell mode\n '),
         fmt('other cmds: prolog_statistics, ls, halt, contexts \n'),!.
...
lp_list:-
      fmt('Listing of the current assertions:\n',[]),
      listing(mooCache),
      fmt('Done.\n',[]).
...
my_call(fol,_):-retractall(version_tag(_)),assert(version_tag(fol)),fmt('% Entering FOL mode.\n',[]),!.
my_call(theoryl,_) :-retractall(version_tag(_)),assert(version_tag(theoryl)),fmt('% Entering ContextL mode.\n',[]),!.

my_call(cd(Term),_Vars):-cd(Term),pwd.

my_call(file(Term),_Vars):-lp_file(Term),fmt('% Entering ask mode.\n',[]),retractall(console_mode(_)),assert(console_mode(ask)).
my_call(file(Term),_Vars):- fmt('% Entering ask mode.\n',[]),retractall(console_mode(_)),assert(console_mode(ask)).
...
%               writeq(Chars),nl,
                ...
                );fmt('could not parse ~w\n',[Code])),once(((once(catch(Term,_,fail)),fmt('Exec Suceeded',[]));fmt('Exec Failed',[])))).
...
/*
writeAnswersUserAgent(UResultsSoFar,Result,InExplaination,Status):-
        fmt('<b>~q.</b>',writeAnswersUserAgent(UResultsSoFar,Result,InExplaination,Status)),!.
*/

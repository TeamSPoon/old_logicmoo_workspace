...
agent_call_command(Agent,actSet(Obj0,Prop0,Value0)) :- coerce(Prop0,tPred,Prop,Prop0),subst(add(dbase_t(Prop,Obj0,Value0)),isSelfAgent,Agent,K),dmsg(K),debugOnError(K).

agent_call_command(Agent,actGet(Obj0,Prop0)) :- subst(dbase_t(Prop0,Obj0,Value),isSelfAgent,Agent,K), 

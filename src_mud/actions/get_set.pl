...
action_info(actGet(isOptional(ftTerm,isAgentSelf),ftTerm),ftText("@get term to a property")).
action_info(actSet(isOptional(ftTerm,isAgentSelf),atom,ftTerm),ftText("@sets term to a property")).
...
agent_call_command(Agent,actSet(Obj0,Prop0,Value0)) :- subst(add(dbase_t(Prop0,Obj0,Value0)),isAgentSelf,Agent,K),dmsg(K),debugOnError(K).

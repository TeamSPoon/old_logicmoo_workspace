...
show_dmsg_values:- listing(tlbugger:dmsg_hidden(_)),current_prolog_flag(opt_debug,ON_OFF),fmt(current_prolog_flag(opt_debug,ON_OFF)),debugging.
...
agent_call_command(_Agent,actDMsg(ON_OFF)):- !, (ON_OFF==vOff->FLAG=false;FLAG=true), set_prolog_flag(debug,FLAG),set_bugger_flag(opt_debug,FLAG),
   fmt(current_prolog_flag(opt_debug,FLAG)).

action_info(actShow(isOptional(prolog,isValueMissing)),"show messages of col").
agent_call_command(_Agent,actShow(A)):-!, (A==isValueMissing -> (set_prolog_flag(debug,true),show_dmsg_values) ; (bugger:dmsg_show(A),prolog_debug:debug(A))).

action_info(actHide(isOptional(prolog,isValueMissing)),"hide messages of col").
agent_call_command(_Agent,actHide(A)):-!, A==isValueMissing -> (set_prolog_flag(debug,false),show_dmsg_values) ; (bugger:dmsg_hide(A),prolog_debug:nodebug(A)).

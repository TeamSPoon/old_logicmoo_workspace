...
action_rules(Agent,use,[Obj],[possess(Agent,Obj),isa(Obj,useable),stowed(Agent,Obj)->using(Agent,Obj)]).
action_rules(Agent,stow,[Obj],[possess(Agent,Obj),isa(Obj,'stowable'),genlPreds(Using,'controls'),[Using,Agent,Obj]]->[stowed,Agent,Obj]).

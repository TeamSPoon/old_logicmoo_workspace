...
user:decl_database_hook(assert(_A_or_Z),mudIsa(W,ttCreateable)):-decl_type_safe(W). %,call_after_game_load(forall(mudIsa(I,W),create_instance(I,W))).
...
ttCompleteExtentAsserted(Ext):- arg(_,vv(tCol,vtDirection,ttFormatType,ftString,genlPreds),Ext).
...
% covered by next 
isa_asserted_nr(I,T):- hasInstance(T,I).   %isa_asserted_0(I,T):-clause(hasInstance(T,I),true). %isa_asserted_0(I,T):-clause(isa(I,T),true).
isa_asserted_nr(I,T):-is_known_trew(mudIsa(I,T)).
isa_asserted_nr(I,T):-isa_from_morphology(I,T).
isa_asserted_nr(I,T):-type_deduced(I,T).
isa_asserted_nr(I,T):- stack_check,fact_loop_checked(mudIsa(I,T),isa_asserted_0(I,T)).
isa_asserted_nr(I,T):-nonvar(T),type_prefix(_Prefix,T),atom(I),!,isa_from_morphology(I,T).
...
is_known_trew(mudIsa(ftInt,ttNotCreatable)).
...
is_known_trew(mudIsa(tCol,ttNotCreatable)).
is_known_trew(mudIsa(tItem,ttCreateable)).
is_known_trew(mudIsa(ttFormatType,ttNotCreatable)).
is_known_trew(mudSubclass(ttFormatType,ttNotCreatable)).
is_known_trew(mudIsa(tSpatialThing, 'ttCreateable')).
is_known_trew(mudIsa(ftTerm,ttNotCreatable)).
...
% is_known_trew(mudSubclass(F,tFunction)):-is_pred_declarer(F).
is_known_trew(mudSubclass(F,tRelation)):-is_pred_declarer(F).
...
is_known_false0(mudIsa(actGossup,tChannel)).
...
not_mud_isa(ttCreateable,tSpatialThing).
not_mud_isa(tSpatialThing,ttCreateable).
...
disjointWith0(ttCreateable,ttNotCreatable).
